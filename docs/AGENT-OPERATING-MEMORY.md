...
4. Prove the verified content is present at every declared path:
   `python3 scripts/oc_verify_merge_landed.py --verified-head <sha> --integration-ref <ref> --base-ref <base-branch> --path <p> [--path ...]`
   Pass a path the change removed as `--deleted-path`. The tool proves it existed at every pre-change revision, because a deletion is only a deletion if the file was there to delete. `--base-ref` names the branch the change forked off; it is required after an ordinary merge, where the verified head is an ancestor of the integration ref and their merge base is therefore the verified head itself. It is validated before use (it must resolve, share an ancestor with the verified head, and not already contain it) and it is used only to derive which paths are probative -- never as a verdict.
...
* **A directory.** `git ls-tree <ref> -- :(literal)src/lib` returns one record, `040000 tree <sha> src/lib`, whose identity changes when a child is DELETED. Declaring a parent directory turned a pure deletion, which the tool refuses outright, into "present" evidence. Only files are accepted. Spell paths exactly as git reports them: a trailing slash matches several entries and resolves to nothing.

**Containment is not evidence.** If the verified head is an ancestor of the integration ref, the COMMIT is in that history and nothing follows about the resulting tree. `git merge -s ours` makes it a parent and lands not one byte; so does `-X ours` on a conflict, and so does a merge followed by a revert. A round of this tool that treated containment as sufficient reported all three as `landed`. It is the reason `--base-ref` exists: not to excuse the probative check, but to make it derivable for the commonest merge shape.

Nine independent reviews found nine false passes, all one shape: a `landed` verdict reached without comparing anything the merge could have changed. Each fix closed a case -- a mistyped path, a dash ref, a deletion needing only absence, a caller-named base, a deletions-only record, a path identical to the merge base, an unresolvable lookup at the base read as a difference, containment treated as sufficient, a directory laundering a deletion into presence -- and the next review found the shape somewhere else, because each fix preserved the premise that some cheap symmetry counts as evidence. When a fix keeps failing in the same shape, the defect is in the premise the fix preserves, not in the case it closed. Twice the fix's own refusal message became the next delivery vector, telling the operator to do the thing that manufactures the pass. Once, the fix for a false REFUSAL became the next false PASS: widening a gate to admit an honest case admitted the dishonest ones with it.
