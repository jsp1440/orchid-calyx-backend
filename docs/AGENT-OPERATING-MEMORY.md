...
So the rule is no longer a constraint on the declaration. The tool derives the set with `git diff --name-status <pre-change revision>..<verified head>` and refuses anything that is not exactly it. A distant base is then self-defeating: it does not shrink the derived set, it grows it, and every path it adds must be declared and compared.
