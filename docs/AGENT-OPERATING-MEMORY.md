...
4. Prove the verified content is present at every declared path:
   ...
   Pass a path the change removed as `--deleted-path`. The tool proves it existed at every `git merge-base --all <verified-head> <integration-ref>`, because a deletion is only a deletion if the file was there to delete, and that base is derived from the two refs being compared rather than named by you.
...
The tool therefore requires one of two things before it compares anything, and the distinction matters when reading a refusal:

* **Containment.** If the verified head is one of the merge bases, it is an ancestor of the integration ref -- the commit is in that history, which is stronger than any path comparison. This is what an ordinary merge or fast-forward looks like. The declared paths are still compared, because a later commit can revert content that genuinely merged.
* **A path the change could have altered.** Otherwise at least one declared `--path` must resolve unambiguously at every merge base and differ from it there. Unresolvable is not different: a pathspec that matches several entries, such as one with a trailing slash, resolves to nothing and cannot witness anything. Spell paths exactly as git reports them.

Seven independent reviews found seven false passes, all one shape: a `landed` verdict reached without comparing anything the merge could have changed. Each fix closed a case -- a mistyped path, a dash ref, a deletion needing only absence, a caller-named base, a deletions-only record, a path identical to the merge base, an unresolvable lookup at the base read as a difference -- and the next review found the shape somewhere else, because each fix preserved the premise that some cheap symmetry counts as evidence. When a fix keeps failing in the same shape, the defect is in the premise the fix preserves, not in the case it closed. Twice the fix's own refusal message became the next delivery vector, telling the operator to do the thing that manufactures the pass.
