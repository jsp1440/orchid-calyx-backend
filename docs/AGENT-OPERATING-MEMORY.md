...
4. Prove the verified content is present at every declared path:
   ...
   **The tool derives the path set itself and refuses any other declaration**, so step 1 is how you find out what to type, not a set you curate. Run it, read the `Declare exactly:` list in the refusal, and pass that.
   `$(git merge-base <verified-head> <base-branch>)` was prescribed here and is **wrong**: it is the fork point only while the base branch carries no commit of this change, and once it carries one the derived set is the change's tail. That is instance twelve, reproduced against shipped code. Pass `--fork-point` as well as `--base-ref`; the tool refuses without it.
...
Instance twelve then came from the *other* half of that sentence — **which revision is the pre-change one.** The tool derived it as the merge base with `--base-ref`, and a base branch that has absorbed any commit of the change makes that merge base a commit inside it. Reproduced against shipped code: a reverted locality fix reported `landed`, exit 0, with the path set derived exactly as the tool's own docstring prescribed. Twice now a fix's own mechanical instruction has been the next delivery vector. `--fork-point` is required, and it can only move the pre-change revision **earlier** — the direction that grows the set.

And instance thirteen, found while fixing twelve, from the same premise read one more way: **configuration is another way of choosing the set.** `git diff --name-status` obeys `git config`. With `diff.ignoreSubmodules=all` — an ordinary setting for anyone who works in a repository with submodules — the gitlink is simply absent from the listing, so a submodule the change bumped need not be declared, and a merge that dropped the bump returns `landed`, exit 0. Reproduced against shipped code. `diff.relative=true` is the milder cousin: run from a subdirectory the derived set is relative to the cwd while `ls-tree --full-tree` reads from the root, and the set empties or shrinks with where someone happened to stand.
