...
PR #818 is the clean current-main integration/validation authority for the `research-station-conversations` profile. Superseded PR #816 is retained only as failure-first history.
...
4. A test-harness import defect: pytest could not import the standalone script through `scripts.*`. The test now loads the activation module by exact file path, allowing the real CLI invocation to remain an independent proof.
5. A migration-140 schema expectation mismatch: canonical `conversation_sessions.title` defaults to `'Calyx conversation'::text`.
6. A direct-script dispatcher import defect: `python scripts/activate_reasoning_prerequisite_schemas.py --profile research-station-conversations` could not resolve `scripts.research_station_conversation_activation`. The dispatcher now imports the sibling module according to its actual direct-script execution model.

### VALIDATED

Exact code head `45687bb1774cfb85c3d13184068a9f4109238d64` passed all triggered release gates on 2026-08-09:

- CALYX Reasoning Prerequisite Activation Validation run `31333792881` — SUCCESS.
...
- BUILD-088E Validation run `31333792876` — SUCCESS.
- CALYX Workflow Governance Audit run `31333792877` — SUCCESS.

The PG15/16/17 jobs each passed:

- pinned migration-byte verification;
- compile, Ruff lint, and format checks;
- eight atomic transaction / rollback / serialization / governance tests;
