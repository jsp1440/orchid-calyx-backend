...
BUILD-049 extends the existing Calyx backend instead of creating a parallel runtime. The implementation adds `runtime.harvester_control` as the governed control-plane service and `app.routers.harvesters` as the authenticated API surface. Existing Mission Control telemetry remains read-only and can consume the richer harvester payload shape.
...
Disable frontend action controls first. Revert the backend router mount to remove action routes. The migration is additive; do not drop tables during rollback unless explicitly approved because run history and proposals are audit records.
