...
def _candidate_snapshot(report: dict[str, Any], limit: int = 5) -> list[dict[str, Any]]:
    ...
    for candidate in (report.get("candidates") or [])[:limit]:
        ...
        snapshot.append({
            ...
            "supporting_characters": [x.get("character") for x in explanations if x.get("status") == "matched"],
            "partial_characters": [x.get("character") for x in explanations if x.get("status") == "partial"],
            "conflicting_characters": [x.get("character") for x in explanations if x.get("status") == "conflict"],
            "missing_characters": [x.get("character") for x in explanations if x.get("status") == "candidate_state_missing"],
            "provenance": candidate.get("provenance"),
        })
    return snapshot


def build_explanation_evidence(evaluation: dict[str, Any], *, audience: Audience, focus: Focus) -> dict[str, Any]:
    ...
    evidence = {
        ...
        "candidate_order": [item.get("taxon_id") for item in report.get("candidates", [])],
...
def _deterministic_narrative(evidence: dict[str, Any]) -> str:
    ...
    if candidates:
        ...
        if first.get("conflicting_characters"):
            lines.append("Conflicting characters: " + ", ".join(map(str, first["conflicting_characters"])) + ".")
        if first.get("missing_characters"):
            lines.append("Missing candidate states reduce coverage and are not treated as biological absence: " + ", ".join(map(str, first["missing_characters"])) + ".")
        if len(candidates) > 1:
            second = candidates[1]
            lines.append(f"The strongest alternative is {second.get('scientific_name')} with score {second.get('score')} and coverage {second.get('coverage')}.")
    else:
    ...
    if next_observation:
        ...
        lines.append(f"The Matrix recommends observing {label} next because it has the strongest deterministic discrimination signal among the remaining unobserved characters.")
    ...
    lines.append("This is an explanation of candidate-ranking evidence, not a verified taxonomic identification.")
    return "\n\n".join(lines)
...
def _provider_prompt(evidence: dict[str, Any]) -> str:
    return (
        ...
        f"Audience: {evidence['audience']}. Focus: {evidence['focus']}. "
        "Explain why the leading candidate currently ranks first, why the strongest alternative differs, and why the deterministic next observation is useful when one is present. "
        "Use plain language first, then botanical terminology where useful. Do not change candidate order, scores, coverage, observations, or the selected next observation. "
        "Do not call scores probabilities. Do not assert a verified identification. Treat missing data as unknown, never as biological absence."
    )
