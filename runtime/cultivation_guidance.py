...
GUIDANCE_FIELDS = (
    "temperature", "light", "water", "humidity", "ventilation", "rest",
    "media", "mounting", "fertilization", "repotting",
)
...
def _semantic_record(record: dict[str, Any]) -> dict[str, Any]:
    """Remove observation metadata so immutable-version replay is content-idempotent."""
    return {key: value for key, value in record.items() if key not in {"created_at", "record_digest"}}
...
class CultivationGuidanceService:
    ...
    def register_guidance(self, owner_id: str, payload: dict[str, Any], *, actor: str) -> dict[str, Any]:
        ...
        semantic = {
            ...
            "locality_context": dict(payload.get("locality_context") or {}),
        ...
        record = {**semantic, "created_at": _now()}
        record["record_digest"] = _digest(semantic)
    ...
    def review_guidance(
        ...
        *,
        ...
        source = self._read(
            self._root(owner_id)
            / "guidance"
            / _safe_id(guidance_id, "CULTIVATION_GUIDANCE_ID_INVALID")
        ...
        return self._write(
            self._root(owner_id) / "reviews" / f"{source['guidance_id']}-v{source['version']}.json",
            decision,
        )
    ...
    def assemble_profile(self, owner_id: str, identity_key: str) -> dict[str, Any]:
        ...
        records = [
            ...
            if key in {
                _text(item["identity"].get("canonical_taxon_id")),
                _text(item["identity"].get("hybrid_identity")),
            }
        ]
        if not records:
    ...
    def conservatory_oasis_handoff(self, owner_id: str, identity_key: str) -> dict[str, Any]:
        return {
            ...
            "guidance_profile": self.assemble_profile(owner_id, identity_key),
    ...
    def readiness(self, owner_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "literature_evidence_count": sum(item["source_kind"] == "literature_evidence" for item in records),
            "grower_observation_count": sum(item["source_kind"] == "grower_observation" for item in records),
            "local_adaptation_count": sum(item["source_kind"] == "local_adaptation" for item in records),
            "evidence_anecdote_separation": True,
            "pesticide_advice_authorized": False,
            "medical_advice_authorized": False,
            "autonomous_greenhouse_control_authorized": False,
