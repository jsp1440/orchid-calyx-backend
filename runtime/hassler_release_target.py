...
_DEFAULT_MANIFEST = Path(__file__).resolve().parents[1] / "config" / "hassler-release-target.json"
...
def load_hassler_release_target(path: str | Path | None = None) -> HasslerReleaseTarget:
    ...
    if isinstance(size_bytes, bool) or not isinstance(size_bytes, int) or size_bytes <= 0:
        raise ValueError("Hassler release manifest requires positive integer size_bytes")

    sha256 = _required_text(payload, "sha256").lower()
    if _SHA256.fullmatch(sha256) is None:
        raise ValueError("Hassler release manifest sha256 must be 64 lowercase hex characters")
    ...
    try:
        ...
        raise ValueError("Hassler release manifest acquired_at must be YYYY-MM-DD") from exc
