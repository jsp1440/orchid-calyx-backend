...
class BrainGapDiagnostics:
    ...
    def gaps(self) -> dict[str, Any]:
        payload = self.diagnose()
        return {"build": "BUILD-017", "status": payload.get("status"), "count": len(payload.get("enriched_gaps", [])), "gaps": payload.get("enriched_gaps", [])}
        ...
        queue = [
            {
                ...
                "task": gap.get("recommended_next_step"),
    ...
    def dashboard(self) -> dict[str, Any]:
        ...
        return {
            ...
            "enriched_gap_count": len(payload.get("enriched_gaps", [])),
            "top_gaps": payload.get("enriched_gaps", [])[:5],
