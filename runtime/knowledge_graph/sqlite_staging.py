...
class SqliteStagingGraphRepository:
    def __init__(self, path: str, *, initialize: bool = True):
        ...
        with sqlite3.connect(self.path) as conn:
            ...
            # Older staging files may have nullable source_table values. Normalize
            # them before enforcing the expression index used for idempotency.
    ...
    @staticmethod
    def _edge(row) -> Edge:
        source_table = row[4] or None
        return Edge(kg_edge_id=row[0], edge_type=row[1], from_node_id=row[2], to_node_id=row[3], source_table=source_table, source_pk=row[5], evidence_class=row[6], confidence_score=row[7], confidence_label=row[8], rule_name=row[9], payload=json.loads(row[10] or "{}"))
