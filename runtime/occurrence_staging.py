...
# License codes accepted for public staging; others enter review queue.
...
class StagedOccurrence:
    """Canonical staged occurrence record with full provenance."""
    ...
    taxon_key: str | None
    canonical_taxon_id: str | None  # resolved from canonical taxonomy or None
    reconciliation_state: str  # "resolved", "unresolved", "review_required"
    latitude: float | None
    ...
    acquisition_checksum: str  # sha256 of canonical fields to detect duplicates
...
class OccurrenceReviewItem:
    """Record that cannot be safely linked to a canonical taxon."""
...
class OccurrenceStagingResult:
    ...
    idempotent: bool  # True when re-run produced no new inserts
...
def _checksum(source: str, source_record_id: str, scientific_name: str) -> str:
    digest = hashlib.sha256(
        f"{source}|{source_record_id}|{scientific_name}".encode()
    ).hexdigest()
    return digest[:16]
...
def _reconcile_taxon(
    ...
    """Return (canonical_taxon_id, reconciliation_state).

    canonical_lookup maps accepted_name -> canonical_taxon_id.
    A None lookup disables live reconciliation (test / dry-run mode).
    """
    if canonical_lookup is None:
        return None, "reconciliation_unavailable"
    accepted = normalized.get("accepted_name") or normalized.get("scientific_name") or ""
    ...
    # Try scientific_name as fallback
    scientific = (normalized.get("scientific_name") or "").strip()
...
def stage_occurrence_batch(
    ...
    *,
    ...
    """Stage one bounded batch of normalized occurrence records.

    Args:
        records: Normalized occurrence dicts from a harvester plugin.
        source: Source name ("gbif" or "inaturalist").
        batch_start: Absolute record offset for checkpoint tracking.
        seen_checksums: Set of already-persisted checksums for idempotency.
        canonical_lookup: Optional accepted_name → canonical_taxon_id mapping.

    Returns:
        OccurrenceStagingResult with staged records, review items, and checkpoint.
    """
    ...
    for record in records_list:
        ...
        # Stage regardless; review items are additive, not blocking
