...
Durable queue, agent registry, run-once executor, observations, evaluations,
and approval gates for Calyx work.
"""
...
except ModuleNotFoundError:  # pragma: no cover - minimal test environments
    psycopg = None
    dict_row = None
    Jsonb = None
...
DEFAULT_AGENTS: list[dict[str, Any]] = [
    {
        "agent_name": "Calyx Auditor",
        "capability": "Read-only repository, runtime, and system health audits.",
        "allowed_task_types": ["frontend_audit", "backend_health_check"],
        "enabled": True,
    },
    {
        "agent_name": "Continuum Data Cartographer",
        "capability": "Read-only coverage audits for image and relationship data.",
        "allowed_task_types": ["image_coverage_audit", "relationship_data_audit"],
        "enabled": True,
    },
    {
    ...
    def execute(self, task: dict[str, Any], agent: dict[str, Any]) -> ExecutionResult:
        ...
        if risky_action:
            return ExecutionResult(
                ...
                [{"event_type": "approval_gate", "action": "skipped", "status": "needs_review", "details": {"risky_action": risky_action}}],
            )
        ...
        observations = [
            {
                ...
                "action": "inspected",
                "status": "completed",
                "details": {"task_type": task_type, "agent": agent["agent_name"], "payload_keys": sorted(payload.keys())},
            }
        ]
        ...
        if task_type == "backend_health_check":
            ...
            return ExecutionResult("completed", self.evaluate(result, ["inspected", "changed", "skipped"]), result, observations)

        if task_type == "frontend_audit":
            result = {
                ...
                "frontend_contract": {
                    ...
                    "tasks": "/api/orchestrator/tasks",
                    "runs": "/api/orchestrator/runs",
                    "observations": "/api/orchestrator/observations",
                },
            }
            return ExecutionResult("completed", self.evaluate(result, ["frontend_contract"]), result, observations)
        ...
        return ExecutionResult(
            "blocked",
            "needs_review",
            {"message": "No executor is registered for this task type.", "task_type": task_type},
            observations + [{"event_type": "executor_resolution", "action": "skipped", "status": "blocked", "details": {"reason": "unsupported_task_type"}}],
        )

    def evaluate(self, result: dict[str, Any], required_keys: list[str]) -> str:
        return "needs_review" if [key for key in required_keys if key not in result] else "pass"

    def risky_action(self, task_type: str, payload: dict[str, Any]) -> Optional[str]:
        for candidate in {task_type, str(payload.get("action", "")), str(payload.get("operation", ""))}:
...
class CalyxAutonomousOrchestrator:
    ...
    def ensure_schema(self, cur) -> None:
        ...
        cur.execute(
            ...
            CREATE TABLE IF NOT EXISTS oc_admin.calyx_tasks (
                ...
                created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
                updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
                CONSTRAINT calyx_tasks_status_check CHECK (status IN ('pending', 'running', 'completed', 'failed', 'blocked', 'needs_review')),
                CONSTRAINT calyx_tasks_eval_check CHECK (evaluation_result IS NULL OR evaluation_result IN ('pass', 'fail', 'needs_review'))
            );
            """
        )
        cur.execute(
    ...
    def _seed_agents(self, cur) -> int:
        ...
        for agent in DEFAULT_AGENTS:
            cur.execute(
                """
                INSERT INTO oc_admin.calyx_agents (agent_name, capability, allowed_task_types, enabled)
                VALUES (%s, %s, %s, %s)
                ...
                """,
                (agent["agent_name"], agent["capability"], _json(agent["allowed_task_types"]), agent["enabled"]),
            )
            inserted += int(bool(cur.fetchone()["inserted"]))
        return inserted
    ...
    def _seed_tasks(self, cur) -> int:
        ...
        for task in DEFAULT_TASKS:
            cur.execute(
                """
                INSERT INTO oc_admin.calyx_tasks (task_key, task_type, title, payload, status, priority, required_approval)
                VALUES (%s, %s, %s, %s, 'pending', %s, FALSE)
                ON CONFLICT (task_key) DO NOTHING
                RETURNING id
                """,
                (task["task_key"], task["task_type"], task["title"], _json(task["payload"]), task["priority"]),
            )
    ...
    def list_agents(self) -> dict[str, Any]:
        with self.connect() as conn:
            with conn.cursor() as cur:
                self.ensure_schema(cur)
                cur.execute("SELECT * FROM oc_admin.calyx_agents ORDER BY enabled DESC, agent_name ASC")
    ...
    def create_task(self, task_type: str, title: str, payload: dict[str, Any], priority: int = 0) -> dict[str, Any]:
        ...
        with self.connect() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    """
                    INSERT INTO oc_admin.calyx_tasks (task_type, title, payload, status, priority, required_approval)
                    VALUES (%s, %s, %s, %s, %s, %s)
                    RETURNING *
                    """,
                ...
                self.log_observation(cur, task_id=task["id"], agent_id=None, event_type="task_created", action="queued" if status == "pending" else "approval_required", status=status, details={"required_approval": required_approval})
            conn.commit()
        return {"task": task}
        ...
        with self.connect() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    ...
                    SET approved_at = NOW(), required_approval = FALSE,
                ...
                self.log_observation(cur, task_id=task_id, agent_id=None, event_type="approval_gate", action="approved", status="pending", details={"approved_at": utc_now()})
            conn.commit()
            ...
            with self.connect() as conn:
                with conn.cursor() as cur:
                    ...
                    for observation in outcome.observations:
                        self.log_observation(cur, task_id=task["id"], agent_id=agent["id"], event_type=observation["event_type"], action=observation["action"], status=observation["status"], details=observation.get("details", {}))
                    self._finish_task(cur, task_id=task["id"], run_id=run_id, status=outcome.status, evaluation_result=outcome.evaluation_result, result=outcome.result, error_text=None)
                conn.commit()
            return {"status": outcome.status, "task_id": task["id"], "task_type": task["task_type"], "agent": agent["agent_name"], "evaluation_result": outcome.evaluation_result, "result": outcome.result}
        except Exception as exc:  # pragma: no cover - defensive runtime guard
            with self.connect() as conn:
                with conn.cursor() as cur:
                    self.ensure_schema(cur)
                    self.log_observation(cur, task_id=task["id"], agent_id=agent["id"], event_type="execution", action="failed", status="failed", details={"error": str(exc)})
                    self._finish_task(cur, task_id=task["id"], run_id=run_id, status="failed", evaluation_result="fail", result={"error": str(exc)}, error_text=str(exc))
                conn.commit()
    ...
    def _select_next_task(self) -> dict[str, Any]:
        with self.connect() as conn:
            with conn.cursor() as cur:
                self.ensure_schema(cur)
                cur.execute("SELECT * FROM oc_admin.calyx_tasks WHERE status = 'pending' ORDER BY priority DESC, id ASC FOR UPDATE SKIP LOCKED")
                tasks = [dict(row) for row in cur.fetchall()]
                if not tasks:
                    return {"status": "no_eligible_tasks"}
                cur.execute("SELECT * FROM oc_admin.calyx_agents WHERE enabled = TRUE ORDER BY id ASC")
                ...
                for task in tasks:
                    ...
                    if risky and not task.get("approved_at"):
                        cur.execute("UPDATE oc_admin.calyx_tasks SET status = 'needs_review', required_approval = TRUE, evaluation_result = 'needs_review', updated_at = NOW() WHERE id = %s", (task["id"],))
                        self.log_observation(cur, task_id=task["id"], agent_id=None, event_type="approval_gate", action="skipped", status="needs_review", details={"risky_action": risky})
                        continue
                    agent = self._agent_for_task(task, agents)
                    if not agent:
                        cur.execute("UPDATE oc_admin.calyx_tasks SET status = 'blocked', evaluation_result = 'needs_review', last_error = 'No enabled agent can execute this task type', updated_at = NOW() WHERE id = %s", (task["id"],))
                        self.log_observation(cur, task_id=task["id"], agent_id=None, event_type="agent_selection", action="blocked", status="blocked", details={"task_type": task["task_type"]})
                        continue
                    cur.execute("UPDATE oc_admin.calyx_tasks SET status = 'running', assigned_agent_id = %s, started_at = NOW(), updated_at = NOW() WHERE id = %s RETURNING *", (agent["id"], task["id"]))
                    selected_task = dict(cur.fetchone())
                    cur.execute("INSERT INTO oc_admin.calyx_runs (task_id, agent_id, status, started_at) VALUES (%s, %s, 'running', NOW()) RETURNING id", (task["id"], agent["id"]))
                    run_id = cur.fetchone()["id"]
                    self.log_observation(cur, task_id=task["id"], agent_id=agent["id"], event_type="agent_selection", action="selected", status="running", details={"agent_name": agent["agent_name"]})
                    conn.commit()
    ...
    def _finish_task(self, cur, *, task_id: int, run_id: int, status: str, evaluation_result: str, result: dict[str, Any], error_text: Optional[str]) -> None:
        cur.execute(
            """
            UPDATE oc_admin.calyx_tasks
            SET status = %s, finished_at = NOW(), updated_at = NOW(), last_error = %s,
                evaluation_result = %s, evaluation_details = %s, result_json = %s
            WHERE id = %s
            """,
            (status, error_text, evaluation_result, _json({"result": evaluation_result, "checked_at": utc_now()}), _json(result), task_id),
        )
        cur.execute("UPDATE oc_admin.calyx_runs SET status = %s, evaluation_result = %s, finished_at = NOW(), result_json = %s, error_text = %s WHERE id = %s", (status, evaluation_result, _json(result), error_text, run_id))

    def log_observation(self, cur, *, task_id: Optional[int], agent_id: Optional[int], event_type: str, action: str, status: str, details: dict[str, Any]) -> None:
        cur.execute("INSERT INTO oc_admin.calyx_observations (task_id, agent_id, event_type, action, status, details) VALUES (%s, %s, %s, %s, %s, %s)", (task_id, agent_id, event_type, action, status, _json(details)))
    ...
    def health(self) -> dict[str, Any]:
        with self.connect() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("SELECT status, COUNT(*) AS count FROM oc_admin.calyx_tasks GROUP BY status")
            ...
            conn.commit()
        return {"status": "ok", "queue_count": counts, "active_agent": dict(active) if active else None, "last_run": dict(last_run) if last_run else None, "failures": failures, "approval_gates": sorted(RISKY_ACTIONS)}
