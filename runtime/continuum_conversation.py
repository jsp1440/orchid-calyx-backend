...
This current-main reconstruction preserves CALYX-634 through CALYX-639 behavior without
...
class ContinuumConversationService:
    ...
    def _evidence_record(result: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "reliability_signals": dict(result.get("reliability_signals") or {}),
    ...
    def _evidence_answer(
        ...
        if not evidence:
            return (
                "I asked the Orchid Continuum, but it returned no eligible evidence for this question. I am not substituting general model knowledge for missing Continuum evidence. The next step is to broaden retrieval or add relevant sources.",
                "unknown",
            )
        if not usable:
            return (
                f"I found {len(evidence)} eligible Continuum record(s) for '{question}', but their display policies do not authorize an excerpt that I can safely synthesize here. I can still show the source metadata and citations for review.",
