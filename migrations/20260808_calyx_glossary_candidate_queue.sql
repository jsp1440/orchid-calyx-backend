...
CREATE TABLE IF NOT EXISTS oc_concepts.glossary_candidate_review_events (
    ...
    candidate_id VARCHAR(64) NOT NULL REFERENCES oc_concepts.glossary_candidates(candidate_id),
    resolution_state VARCHAR(40) NOT NULL,
    reviewed_concept_id UUID NULL REFERENCES oc_concepts.concepts(concept_id),
    ...
    CONSTRAINT ck_glossary_review_event_final_state CHECK (
        resolution_state IN ('REVIEWED_MATCH', 'NEW_CONCEPT_CANDIDATE', 'REJECTED')
    )
);
...
CREATE TABLE IF NOT EXISTS oc_concepts.glossary_figure_requests (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    revised_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT ck_glossary_figure_subject CHECK (
        concept_id IS NOT NULL OR source_candidate_id IS NOT NULL
    ),
    CONSTRAINT ck_glossary_figure_context CHECK (
        scientific_context_status IN ('CANONICAL_CONCEPT', 'CANDIDATE_REQUIRES_REVIEW')
    ),
    ...
    CONSTRAINT ck_glossary_figure_review_required CHECK (review_required = TRUE),
...
CREATE INDEX IF NOT EXISTS ix_glossary_figure_requests_candidate
    ON oc_concepts.glossary_figure_requests(source_candidate_id, created_at);
...
COMMENT ON TABLE oc_concepts.glossary_figure_requests IS
    'Human-review figure production queue linked to canonical concepts or unresolved glossary candidates; prompts are draft production aids, never scientific evidence.';
