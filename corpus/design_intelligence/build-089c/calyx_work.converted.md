...
![Calyx fourteen-stage ingestion pipeline (flowchart). Stages: 1 Registry census → 2 Identify-probe → 3 Incremental harvest → 4 Tombstone processing and 5 Rights engine → 6 Decision routing → 7 Registration → 8 Full-text acquisition → 9 Dedupe and merge → 10 Structure parse → 11 Entity/reasoning extraction → 12 Cross-reference → 13 Review queue → 14 Knowledge publication; decision routing also feeds the review queue directly for metadata-only and human-review records.](charts/sec04_pipeline_mermaid.png)
