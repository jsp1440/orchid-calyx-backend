...
class Ed25519OwnerGrantSignatureVerifier:
    ...
    def __init__(
        self,
        *,
        keys: Mapping[str, OwnerVerificationKey],
        ...
        unknown_revoked = revoked - normalized.keys()
        if unknown_revoked:
    ...
    def from_environ(
        ...
        try:
            for key_id, public_key in decoded.items():
                ...
                keys[normalized_id] = OwnerVerificationKey.from_base64url(
                    key_id=normalized_id,
        ...
        revoked = frozenset(
            item.strip() for item in raw_revoked.split(",") if item.strip()
        )
        try:
    ...
    def active_key_ids(self) -> tuple[str, ...]:
        return tuple(
            sorted(key_id for key_id in self._keys if key_id not in self._revoked_key_ids)
        )
