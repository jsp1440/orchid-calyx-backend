...
class ReviewRegistry:
    ...
    def eligibility(self, request_id: str) -> ReleaseEligibility:
        ...
        by_class = {d.review_class: d for d in self.decisions.values() if d.request_id == request_id}
        ...
        code = "REVIEW_REJECTED" if rejected else "CHANGES_REQUESTED" if changes else "REVIEWS_PENDING" if pending else "RELEASE_ELIGIBLE"
    ...
    def mission_control_queue(self) -> tuple[dict[str, object], ...]:
        return tuple({"request_id": request_id, "artifact_id": result.artifact_id, "eligible": result.eligible, "code": result.code, "pending_classes": list(result.pending_classes), "rejected_classes": list(result.rejected_classes), "changes_requested_classes": list(result.changes_requested_classes)} for request_id in sorted(self.requests) for result in (self.eligibility(request_id),))
