...
class ImmutableArtifactRegistry:
    """In-memory reference implementation for immutable artifact registration.

    The registry stores metadata and lineage only. It does not publish, approve,
    interpret, or mutate scientific conclusions.
    """
    ...
    def register(self, registration: ArtifactRegistration) -> RegistrationResult:
        ...
        if existing is not None:
            ...
            return RegistrationResult(record=existing, duplicate_content_of=None, created=False)
        ...
        return RegistrationResult(record=record, duplicate_content_of=duplicate, created=True)

    def relate(
        self,
        *,
        source_artifact_id: str,
        relation: ArtifactRelationType | str,
        target_artifact_id: str,
    ) -> ArtifactRelation:
    ...
    def require_evidence(self, artifact_id: str) -> ArtifactRecord:
        ...
        evidence_relations = tuple(
            edge for edge in self._relations
            if edge.source_artifact_id == artifact_id and edge.relation == ArtifactRelationType.EVIDENCES
        )
        if not record.evidence_uris and not evidence_relations:
    ...
    def lineage(self, artifact_id: str) -> tuple[ArtifactRelation, ...]:
        ...
        return tuple(
            sorted(
                (edge for edge in self._relations if artifact_id in {edge.source_artifact_id, edge.target_artifact_id}),
                key=lambda edge: (edge.source_artifact_id, edge.relation.value, edge.target_artifact_id),
            )
        )
    ...
    def snapshot(self) -> dict[str, object]:
        return {
            ...
            "unique_content_count": len(self._by_checksum),
            ...
            "artifacts": [
                {
                    ...
                    "evidence_uris": list(record.evidence_uris),
                }
    ...
    def _to_record(registration: ArtifactRegistration) -> ArtifactRecord:
        return ArtifactRecord(
            ...
            byte_length=len(registration.content),
            media_type=registration.media_type,
            source_uri=registration.source_uri,
            producer_assignment_id=registration.producer_assignment_id,
            license=registration.license,
            evidence_uris=_dedupe(registration.evidence_uris),
