...
PROHIBITED_ACTIONS = frozenset(
    {
        ...
        "mutate_production_knowledge_graph",
    }
)
...
def _verify_manifest_non_authority(snapshot: Mapping[str, Any]) -> None:
    if snapshot.get("schema") != MANIFEST_SCHEMA:
...
class GitMutationAuthorizationGate:
    """Verify exact owner authorization without performing any Git or network mutation."""
    ...
    def build_request(
        ...
        *,
        ...
        normalized_actions = tuple(dict.fromkeys(str(action).strip() for action in actions))
        if not normalized_actions or any(action not in ALLOWED_ACTIONS for action in normalized_actions):
            raise PermissionError("GIT_AUTHORIZATION_ACTION_NOT_ALLOWED")
        if any(action in PROHIBITED_ACTIONS for action in normalized_actions):
            raise PermissionError("GIT_AUTHORIZATION_PROHIBITED_ACTION")

        repository = str(manifest_snapshot.get("repository") or "").strip()
        base_commit = str(manifest_snapshot.get("base_commit_sha") or "").strip().lower()
    ...
    def sign_for_test_or_operator(
        ...
        *,
        ...
        """Deterministically sign an explicit decision; this method performs no Git mutation."""
        unsigned = {
            "schema": GRANT_SCHEMA,
