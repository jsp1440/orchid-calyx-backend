...
JOURNAL_SCHEMA = "calyx-git-proposal-mutation-journal-event-v3"
FINAL_STATUSES = frozenset({"completed", "completed_subset"})
ALLOWED_STATUSES = FINAL_STATUSES | {"in_progress", "partial_failure", "failed"}
...
class GitProposalMutationJournalEventRecord(Base):
    ...
    patch_program_job_id: Mapped[str] = mapped_column(String(128), index=True)
...
class DurableGitProposalMutationJournal:
    """Append-only, tamper-checked proposal mutation journal for restart recovery."""
    ...
    def recovery_state(
    ...
    ) -> GitProposalMutationRecoveryState:
        ...
        if (
            latest.patch_program_job_id != plan.patch_program_job_id
            or latest.repository != plan.repository
            or latest.proposed_branch != plan.proposed_branch
            or latest.base_commit_sha != plan.base_commit_sha
            or latest.base_ref != plan.base_ref
        ):
            raise PermissionError("GIT_PROPOSAL_JOURNAL_PLAN_IDENTITY_MISMATCH")
        expected_prefix = tuple(
            operation.action
            for operation in plan.operations[: len(latest.completed_actions)]
        )
        ...
        if latest.status == "completed":
            ...
            classification = (
                "resumable_partial" if latest.completed_actions else "not_started"
            )
            terminal = False
        return GitProposalMutationRecoveryState(
    ...
    def _validate_snapshot(snapshot: Mapping[str, Any]) -> None:
        ...
        if not patch_job:
            raise PermissionError("GIT_PROPOSAL_JOURNAL_PATCH_JOB_INVALID")
        if payload.get("status") not in ALLOWED_STATUSES:
            raise PermissionError("GIT_PROPOSAL_JOURNAL_STATUS_INVALID")
        if not str(payload.get("base_ref") or "").strip():
            raise PermissionError("GIT_PROPOSAL_JOURNAL_BASE_REF_INVALID")
    ...
    def _decode(
        ...
        if (
            raw.get("plan_digest") != row.plan_digest
            or raw.get("patch_program_job_id") != row.patch_program_job_id
            or raw.get("receipt_digest") != row.receipt_digest
        ):
            raise PermissionError("GIT_PROPOSAL_JOURNAL_ROW_IDENTITY_MISMATCH")
        try:
            evidence = tuple(
            ...
            receipt = GitProposalMutationReceipt(
                ...
                failure_code=(
                    None
                    if raw.get("failure_code") is None
                    else str(raw["failure_code"])
                ),
            )
