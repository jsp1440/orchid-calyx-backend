...
def matrix_identification_table(report: dict[str, Any]) -> dict[str, Any] | None:
    ...
    for rank, candidate in enumerate(candidates[:20], start=1):
        ...
        rows.append(
            {
                "rank": rank,
                "scientific_name": _text(candidate.get("scientific_name"), 220),
                "taxon_id": _text(candidate.get("taxon_id"), 180),
                "match_percent": round(float(score) * 100, 1) if isinstance(score, (int, float)) else "",
                "coverage_percent": round(float(coverage) * 100, 1) if isinstance(coverage, (int, float)) else "",
                "compared_weight": candidate.get("compared_weight") if isinstance(candidate.get("compared_weight"), (int, float)) else "",
            }
        )
    if not rows:
        return None
    ...
    return {
        ...
        "subtitle": (
            "Derived from the submitted observation/candidate matrix. Ranking supports review and does not assert a verified identification."
        ),
        "provenance": {
