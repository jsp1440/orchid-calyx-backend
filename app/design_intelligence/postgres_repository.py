...
class PostgresDesignCorpusRepository:
    ...
    def _document(row) -> DesignDocument:
        return DesignDocument(
            ...
            domains=tuple(
                sorted((DesignDomain(item) for item in row["domains"]), key=str)
            ),
            ...
            cur.execute(
                ...
                (
                    ...
                    digest(
                        {
                            "document": document_id,
                            "content": document.provenance.content_hash,
                        }
                    ),
                ),
            )
    ...
    def review_state(self, document_id: int) -> ReviewState:
        with self._connect() as con, con.cursor() as cur:
            cur.execute(
                "SELECT state FROM oc_design_intelligence.review_events WHERE document_id=%s ORDER BY review_event_id DESC LIMIT 1",
                (document_id,),
            )
            row = cur.fetchone()
    ...
    def publish(self, document_id, status, actor, rationale):
        ...
        with self._connect() as con, con.cursor() as cur:
            cur.execute(
                "INSERT INTO oc_design_intelligence.publication_events(document_id,status,actor,rationale) VALUES(%s,%s,%s,%s) RETURNING *",
                (document_id, status.value, actor, rationale),
            )
            result = dict(cur.fetchone())
            self._audit(
                cur,
                document_id,
                "PUBLICATION_STATUS_APPENDED",
                actor,
                {"status": status},
            )
            return result
    ...
    def publication_status(self, document_id: int) -> PublicationStatus:
        with self._connect() as con, con.cursor() as cur:
            cur.execute(
                "SELECT status FROM oc_design_intelligence.publication_events WHERE document_id=%s ORDER BY publication_event_id DESC LIMIT 1",
                (document_id,),
            )
            row = cur.fetchone()
    ...
    def published_latest(self) -> list[DesignDocument]:
        ...
        return [
            ...
            if self.publication_status(item.document_id) is PublicationStatus.PUBLISHED
        ]

    @staticmethod
    def _audit(cur, document_id, event_type, actor, details):
        cur.execute(
            "INSERT INTO oc_design_intelligence.audit_events(document_id,event_type,actor,details) VALUES(%s,%s,%s,%s)",
            (document_id, event_type, actor, Jsonb(details)),
        )
