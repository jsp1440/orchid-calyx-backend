...
class MemoryIndexRepository:
 ...
 def persist(self,item,vector,tokens):
  ...
  run=self.runs[item["run_id"]]; ident=self._next(); d={"index_document_id":ident,"source_object_type":item["document"].source_object_type,"source_object_id":item["document"].source_object_id,"revision_id":item["document"].revision_id,"parent_id":item["document"].parent_id,"anchors":item["document"].source_anchor_ids,"content_hash":item["content_hash"],"metadata_hash":item["metadata_hash"],"model_id":run["model_id"],"configuration_hash":run["configuration_hash"],"active":True,"version":1+(old["version"] if old else 0)}; self.documents.append(d); self.vectors.append({"index_document_id":ident,"vector":vector,"dimension":len(vector),"active":True,"run_id":item["run_id"]}); self.lexical.append({"index_document_id":ident,"normalized_text":item["document"].text.casefold(),"language":item["document"].language,"title":item["document"].title}); item["state"]="INDEXED"; run["metrics"].update(indexed=run["metrics"]["indexed"]+1,tokens=run["metrics"]["tokens"]+tokens,provider_calls=run["metrics"]["provider_calls"]+1,failed=max(0,run["metrics"]["failed"]-(1 if was_failed else 0)))
