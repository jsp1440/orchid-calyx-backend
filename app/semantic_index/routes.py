...
try:
 _activate_repository(_build_repository())
except Exception:
 _mark_unavailable()
...
def _write(operation):
 ...
 try:
  result = repository.atomic(operation) if hasattr(repository,"atomic") else operation()
  if DURABLE_MODE_CONFIGURED and hasattr(repository, "atomic"):
...
def retrieval_backend_status():
 ...
 if REPO is not None:
  ...
  authorized = sum(1 for d in REPO.documents if d.get("metadata",{}).get("display_policy") not in (None,"UNKNOWN_REQUIRES_REVIEW") and d.get("active",False))
  active_models = len(REPO.models)
 elif not DURABLE_MODE_CONFIGURED:
  backend = "MemoryIndexRepository"
 return {"retrieval_backend":backend,"durable":durable,"degraded":degraded,"unavailable":degraded,"indexed_document_count":indexed,"display_authorized_count":authorized,"active_model_count":active_models,"index_error":REPO_ERROR,"last_successful_durable_refresh_at":LAST_DURABLE_REFRESH_AT,"last_successful_durable_write_at":LAST_DURABLE_WRITE_AT}
...
def preview(p:PreviewIn): _,service=_ensure_repository(); return _write(lambda:service.preview([IndexDocument(**{**x.model_dump(),"source_anchor_ids":tuple(x.source_anchor_ids)}) for x in p.documents],configuration=p.configuration))
@router.post("/runs/{run_id}/execute")
def execute(run_id:int): _,service=_ensure_repository(); return _write(lambda:service.execute(run_id))
@router.get("/runs/{run_id}")
def status(run_id:int): return _read().status(run_id)
@router.post("/runs/{run_id}/cancel")
def cancel(run_id:int): _,service=_ensure_repository(); return _write(lambda:service.cancel(run_id))
@router.post("/runs/{run_id}/resume")
def resume(run_id:int): _,service=_ensure_repository(); return _write(lambda:service.resume(run_id))
@router.get("/history")
def history(): return {"items":list(_read().runs.values())}
@router.get("/runs/{run_id}/items")
def items(run_id:int): repository=_read(); return {"items":repository.items[run_id],"warnings":repository.warnings}
@router.get("/registry")
def registry(): return {"models":_read().models}
@router.get("/reviews")
def reviews(): return {"items":_read().reviews}
@router.get("/sources/{object_type}/{object_id}/versions")
def versions(object_type:str,object_id:int): return {"items":[x for x in _read().documents if x["source_object_type"]==object_type and x["source_object_id"]==object_id]}
@router.get("/tombstones")
def tombstones(): return {"items":_read().tombstones}
