...
class MemoryCandidateRepository:
    ...
    def create_run(
        self, configuration_hash: str, extractor_version: str, ruleset_version: str
    ) -> int:
        run_id = self.next_id()
        self.runs[run_id] = {
    ...
    def add_item(
        self, run_id: int, evidence: Any, fingerprint: str, action: str
    ) -> None:
        item_id = self.next_id()
        self.items[run_id].append(
            {
                "item_id": item_id,
                "run_id": run_id,
                "evidence": evidence,
    ...
    def pending(self, run_id: int) -> list[dict[str, Any]]:
        return [
            ...
            if item["state"] in {"PLANNED", "FAILED"}
        ]
    ...
    def candidate_for_fingerprint(self, fingerprint: str) -> dict[str, Any] | None:
        return next(
            (
                ...
                if candidate["evidence_fingerprint"] == fingerprint
            ),
            None,
        )
    ...
    def active_identity(
        self, kind: str, subject: str, predicate: str
    ) -> list[dict[str, Any]]:
        return [
            candidate
            for candidate in self.candidates
            ...
            and candidate["normalized_subject"] == subject
    ...
    def resolve_review(
        self, review_id: int, decision: str, rationale: str, actor: str
    ) -> dict[str, Any]:
        if decision not in {
            "APPROVE_CANDIDATE",
            "REJECT_CANDIDATE",
        ...
        candidate = next(
            (
                ...
                if item["candidate_id"] == review.get("candidate_id")
            ),
            None,
        )
        if candidate:
