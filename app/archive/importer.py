...
class ArchiveImporter:
    def __init__(self, registry: ArchiveRegistry | None = None, scanner: ArchiveScanner | None = None,
                 extractor: DocumentExtractor | None = None, entity_extractor: EntityExtractor | None = None,
                 relationship_extractor: RelationshipExtractor | None = None) -> None:
    ...
    def resume(self, run_id: uuid.UUID) -> uuid.UUID:
        ...
        self._run(run_id, Path(run["source_path"]), options, start_index=int(checkpoint["next_file_index"]))
        return run_id

    def _discover(self, source: Path, options: ImportOptions, temp_root: Path) -> list[ScannedFile]:
    ...
    def _run(self, run_id: uuid.UUID, source: Path, options: ImportOptions, *, start_index: int) -> None:
        started = monotonic()
        try:
            with TemporaryDirectory(prefix="calyx-archive-") as temp:
                ...
                for index, item in enumerate(files[start_index:], start=start_index):
                    try:
                    ...
                    except Exception as exc:  # bad files never abort a run
                        ...
                        if next_index % options.checkpoint_interval == 0:
                            self.checkpoints.save(run_id, next_index=next_index, relative_path=item.relative_path,
                                                  state={"elapsed_seconds": monotonic() - started})
                self.checkpoints.save(run_id, next_index=len(files), relative_path=files[-1].relative_path if files else None,
