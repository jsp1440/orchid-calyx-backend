...
class KnowledgeGraphExporter(Protocol):
    def export_entities_and_relationships(self, document_id: str) -> dict[str, Any]: ...
...
class ArchiveSearch:
    ...
    def documents(self, *, limit: int = 100, offset: int = 0) -> list[dict[str, Any]]:
        with self.registry.connection() as conn:
            return list(conn.execute("SELECT * FROM archive_documents ORDER BY created_at DESC LIMIT %s OFFSET %s", (limit, offset)).fetchall())

    def entities(self, *, limit: int = 100, offset: int = 0) -> list[dict[str, Any]]:
        with self.registry.connection() as conn:
            return list(conn.execute("SELECT * FROM archive_entities ORDER BY created_at DESC LIMIT %s OFFSET %s", (limit, offset)).fetchall())
