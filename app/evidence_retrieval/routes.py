...
ENGINE=RetrievalEngine(None,DeterministicLocalProvider())
...
def run(p,mode,extra=None):
 ...
 except RuntimeError as e: raise HTTPException(503,detail={"code":"SEMANTIC_INDEX_DATABASE_UNAVAILABLE"}) from e
@router.post("/lexical")
...
def one(index_document_id:int): return next((x for x in _repo().documents if x["index_document_id"]==index_document_id),None)
...
@router.get("/health")
def health(): return {"status":"ok","read_only":True,"active_models":len(_repo().models),"ranking_version":ENGINE.ranking_version}
@router.get("/status")
def status():
 try:
  _repo()
