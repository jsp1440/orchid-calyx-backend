...
class ScientificEvidenceRouter:
    ...
    Primary-route selection is title-aware: an explicit title-level scientific domain
    is treated as stronger descriptive evidence of the paper's main purpose than a
    secondary method or context mentioned only in the abstract. Other detected domains
    are retained as secondary review routes rather than discarded.
    """
    ...
    def _title_text(article: dict[str, Any]) -> str:
        return str(article.get("title") or "").casefold()
    ...
    def _secondary_routes(
        *,
        primary: str,
        phylogenetic: list[str],
        morphology: list[str],
        pollination: list[str],
        genomic: list[str],
        molecular: list[str],
        has_gene_annotations: bool,
    ) -> tuple[str, ...]:
        routes: list[str] = []
        if morphology and primary != "trait_morphology_evidence":
            routes.append("trait_morphology_evidence")
        if phylogenetic and primary != "phylogenetic_sequence_context":
            routes.append("phylogenetic_sequence_context")
        if genomic and primary != "genomic_resource":
            routes.append("genomic_resource")
        if pollination and primary not in {"pollination_ecology_evidence", "pollinator_selection_context"}:
            routes.append("pollination_ecology_evidence")
        if (has_gene_annotations or molecular) and primary not in {
            "molecular_association_candidate",
            "molecular_context",
            "genomic_resource",
        }:
            routes.append("molecular_context")
    ...
    def route(
        ...
        *,
        ...
        title = self._title_text(article)
        ...
        title_phylogenetic = self._matches(title, self.PHYLOGENETIC_TERMS)
        title_morphology = self._matches(title, self.MORPHOLOGY_TERMS)
        title_pollination = self._matches(title, self.POLLINATION_TERMS)
        title_genomic = self._matches(title, self.GENOMIC_RESOURCE_TERMS)
        ...
        if molecular_candidate_count > 0:
            return RoutedPublication(
                ...
                secondary_routes=self._secondary_routes(
                    primary="molecular_association_candidate",
                    phylogenetic=phylogenetic,
                    morphology=morphology,
                    pollination=pollination,
                    genomic=genomic,
                    molecular=molecular,
                    has_gene_annotations=has_gene_annotations,
                ),
            )

        # Explicit title-level domains describe the paper's principal scientific purpose
        # more reliably than secondary methods or context mentioned in the abstract.
        if title_morphology and not title_phylogenetic:
            reasons = ["title_level_morphology_signal"] + title_morphology[:3]
            ...
            return RoutedPublication(
                ...
                secondary_routes=self._secondary_routes(
                    primary="trait_morphology_evidence",
                    phylogenetic=phylogenetic,
                    morphology=morphology,
                    pollination=pollination,
                    genomic=genomic,
        ...
        if title_phylogenetic and not title_morphology:
            reasons = ["title_level_phylogenetic_signal"] + title_phylogenetic[:3]
            ...
            return RoutedPublication(
                ...
                reasons=tuple(reasons),
                secondary_routes=self._secondary_routes(
                    primary="phylogenetic_sequence_context",
                    phylogenetic=phylogenetic,
                    morphology=morphology,
                    pollination=pollination,
                    genomic=genomic,
        ...
        if title_genomic:
            reasons = ["title_level_genomic_resource_signal"] + title_genomic[:3]
            if has_gene_annotations:
        ...
        if title_pollination and association and (has_gene_annotations or molecular or genomic):
            return RoutedPublication(
                route="pollinator_selection_context",
                confidence=0.85,
                reasons=tuple(
                    ["title_level_pollination_signal", "selection_or_association_terms"]
                    + title_pollination[:2]
                    + association[:2]
                ),
                secondary_routes=self._secondary_routes(
                    primary="pollinator_selection_context",
                    phylogenetic=phylogenetic,
                    morphology=morphology,
                    pollination=pollination,
                    genomic=genomic,
        ...
        if pollination:
            return RoutedPublication(
                ...
                secondary_routes=self._secondary_routes(
                    primary="pollination_ecology_evidence",
                    phylogenetic=phylogenetic,
                    morphology=morphology,
                    pollination=pollination,
                    genomic=genomic,
