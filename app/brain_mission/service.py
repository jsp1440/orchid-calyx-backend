...
class BrainMissionService:
    ...
    def start(
        ...
        *,
        ...
        started = self.clock()
        context = {**mission, "tenant_id": tenant_id, "project_id": project_id, "actor": actor}
        stages = (
        ...
        for stage, component_name in stages:
            ...
            if component is None:
                self._block(mission, f"{component_name.upper()}_COMPONENT_UNAVAILABLE", stage)
                break
        ...
        if not mission["blockers"] and mission["current_stage"] == "eligible_for_publication_state":
            mission["state"] = "AWAITING_HUMAN_REVIEW" if mission["review_status"] != "APPROVED" else "COMPLETE"
    ...
    def _plan(question: str, max_sources: int) -> dict[str, Any]:
        domains = ["taxonomy", "geographic_distribution", "pollination_biology", "conservation", "mycorrhiza"]
        return {
            "question": question,
            "domains": domains,
            "retrieval_queries": [f"{question} {domain.replace('_', ' ')}" for domain in domains],
    ...
    @staticmethod
    def _block(mission: dict[str, Any], code: str, stage: str, detail: str | None = None) -> None:
        blocker = {"code": code, "stage": stage}
        if detail:
            blocker["detail"] = detail[:500]
        mission["blockers"].append(blocker)
        mission["validation"] = {"valid": False, "blockers": [item["code"] for item in mission["blockers"]]}
        mission["publication_eligibility"] = {
            "eligible": False,
            "automatic_publication": False,
            "blockers": [item["code"] for item in mission["blockers"]] + ["HUMAN_REVIEW_REQUIRED"],
        }
    ...
    def _apply(stage: str, output: Any, mission: dict[str, Any], max_sources: int) -> None:
        ...
        if stage == "evidence_retrieval":
            ...
            mission["contradicting_evidence"] = list(output.get("contradicting_evidence", []))
        elif stage == "contradiction_and_gap_analysis":
            mission["contradicting_evidence"] = list(output.get("contradicting_evidence", mission["contradicting_evidence"]))
            ...
            mission["reasoning_ledger"] = {"ledger_id": str(ledger_id), "version": version}
        elif stage == "validation":
            mission["validation"] = {"valid": bool(output.get("valid")), "blockers": list(output.get("blockers", []))}
        elif stage == "human_review_state":
            mission["review_status"] = str(output.get("status") or "HUMAN_REVIEW_REQUIRED")
        elif stage == "eligible_for_publication_state":
            eligible = bool(output.get("eligible")) and mission["review_status"] == "APPROVED" and mission["validation"]["valid"]
            mission["publication_eligibility"] = {
                "eligible": eligible,
                "automatic_publication": False,
                "blockers": [] if eligible else list(output.get("blockers", [])) or ["HUMAN_REVIEW_REQUIRED"],
            }
