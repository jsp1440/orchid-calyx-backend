...
def _retrieve(
    context: dict[str, Any], engine: RetrievalEngine = ENGINE
) -> dict[str, Any]:
    ...
    for query in context["plan"]["retrieval_queries"]:
        ...
        response = engine.search(
            RetrievalQuery(text=query, mode="HYBRID", limit=remaining)
        )
...
class ExistingBrainMissionAdapter:
    ...
    def _canonical_source(result: dict[str, Any]) -> EvidenceInput:
        ...
        if any(
            not isinstance(canonical.get(name), int) or canonical[name] <= 0
        ...
        return EvidenceInput(
            source_object_type=str(
                canonical.get("source_object_type") or result.get("object_type") or ""
            ),
        ...
        return {
            ...
            "artifacts": {
                ...
                "candidate_confidences": {
                    str(key): value.confidence for key, value in by_id.items()
                },
            },
        }
        ...
        for value in context["artifacts"]["canonical_evidence"]:
            anchors = tuple(
                SourceAnchorReference(
                    ...
                    content_hash=value["metadata"].get("content_hash")
            ...
            sources.append(
                SourceEvidenceReference(
                    ...
                    provenance=value["metadata"].get("provenance")
    ...
    def create_ledger(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        for index, (kind, text, confidence, attributes) in enumerate(records):
            ledger = self.ledgers.append(
                ...
                LedgerEntry(
                    entry_id=uuid5(
                        NAMESPACE_URL, f"{context['mission_id']}:{kind.value}:{index}"
                    ),
                    kind=kind,
                    text=text,
    ...
    def validate(self, context: dict[str, Any]) -> dict[str, Any]:
        blockers = self.ledgers.validate(
            context["reasoning_ledger"]["ledger_id"], tenant_id=context["tenant_id"]
        )
        structural = [value for value in blockers if "human approval" not in value]
        return {
            "valid": not structural,
            "blockers": structural,
            "artifacts": {"publication_gate_blockers": blockers},
        }
    ...
    def publication_eligibility(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "eligible": ledger.is_publishable,
            "blockers": context["artifacts"].get(
                "publication_gate_blockers", ["HUMAN_REVIEW_REQUIRED"]
            ),
        }
