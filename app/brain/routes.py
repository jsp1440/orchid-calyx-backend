...
def relationships(node_id: int, repository: GraphRepositoryDependency, edge_type: Annotated[str | None, Query(min_length=1, max_length=100)] = None, limit: Annotated[int, Query(ge=1, le=500)] = 100) -> dict[str, Any]:
    if repository.get_node(node_id) is None:
        raise HTTPException(404, detail={"code": "NODE_NOT_FOUND"})
    edges = repository.get_outgoing_edges([node_id], [edge_type] if edge_type else None, limit=limit)
...
def _infer(repository, subject_node_id: int, inference_type: InferenceType, limit: int):
    try:
        return InferenceEngine(repository).infer(subject_node_id, inference_type, limit=limit)
...
@router.get("/reason")
def reason(subject_node_id: Annotated[int, Query(gt=0)], inference_type: Annotated[InferenceType, Query()], repository: GraphRepositoryDependency, limit: Annotated[int, Query(ge=1, le=100)] = 25) -> dict[str, Any]:
    return _infer(repository, subject_node_id, inference_type, limit)


@router.post("/infer")
def infer(request: InferRequest, repository: GraphRepositoryDependency) -> dict[str, Any]:
    return _infer(repository, request.subject_node_id, request.inference_type, request.limit)


@router.post("/reasoning-map")
def reasoning_map(request: ReasoningMapRequest, repository: GraphRepositoryDependency) -> dict[str, Any]:
    """Build a deterministic, evidence-bearing causal pathway over the Knowledge Graph."""
    try:
        return ReasoningMapEngine(repository).build(request.subject_node_id, direction=request.direction, profile=request.profile, max_depth=request.max_depth, limit=request.limit, edge_types=request.edge_types, causal_only=request.causal_only)
    except Exception as exc:
        _translate(exc)
        raise
...
def scoped_reasoning_map(request: ScopedReasoningMapRequest, repository: GraphRepositoryDependency) -> dict[str, Any]:
    """Evaluate causal pathways against an explicit applicability scope."""
    try:
...
@router.post("/inferences/{subject_node_id}/submit-to-ledger", status_code=201)
def submit_inference_to_ledger(subject_node_id: int, payload: InferenceLedgerSubmission, request: Request, auth: AuthDependency, db: DbDependency, repository: GraphRepositoryDependency) -> dict[str, Any]:
    owner = _subject(auth)
    result = _invoke(db, request, lambda: InferenceLedgerBridge(db, repository).submit(ledger_id=str(payload.ledger_id), project_id=str(payload.project_id), owner=owner, expected_version=payload.expected_version, subject_node_id=subject_node_id, inference_type=payload.inference_type, candidate_node_id=payload.candidate_node_id, inference_content_hash=payload.inference_content_hash))
...
def query(request: GraphQuery, repository: GraphRepositoryDependency) -> dict[str, Any]:
    ...
    if request.edge_type:
        edges = [edge for edge in edges if edge.edge_type == request.edge_type and edge.from_node_id in node_ids]
    return {"nodes": [node.to_dict() for node in sorted(nodes, key=lambda item: item.kg_node_id)[: request.limit]], "edges": [edge.to_dict() for edge in sorted(edges, key=lambda item: item.kg_edge_id)[: request.limit]], "limit": request.limit, "read_only": True}


@router.post("/connect")
def connect(request: ConnectRequest, registry: Annotated[ConnectorRegistry, Depends(get_connector_registry)]) -> dict[str, Any]:
    try:
