...
class ReviewedEvidenceClassificationService:
    """Apply explicit human/reviewer design classifications to matrix rows."""
    ...
    def apply(
        ...
        for decision in decisions:
            ...
            if decision.evidence_id not in indexed:
            ...
            row = indexed[decision.evidence_id]
...
class CrossStudySynthesisService:
    """Create conservative, traceable claims from source-bound evidence rows."""

    def synthesize(
        self, rows: tuple[EvidenceMatrixRow, ...]
        ...
        for (taxon, outcome), members in sorted(groups.items()):
            ...
            claims.append(
                SynthesisClaim(
                    claim_id=self._claim_id(taxon, outcome, support_ids),
                    text=text,
    ...
    @staticmethod
    def _claim_id(taxon: str, outcome: str, support_ids: tuple[str, ...]) -> str:
        digest = hashlib.sha256(
            repr((taxon, outcome, support_ids)).encode("utf-8")
        ).hexdigest()[:20]
        return f"synthesis:{digest}"


class GroundedScientificAuthoringService:
    """Render an article from claims without inventing scientific sentences."""
    ...
    def author(
        ...
        *,
        ...
        if not question.strip() or not title.strip() or not audience.strip() or not format.strip():
        ...
        known_sources = {record.source_id for record in bibliography}
        if source_ids - known_sources:
        ...
        sentences.append(
            ArticleSentence(
                ...
                text=(
                    "The conclusions above are constrained to the verified, source-bound "
                    "evidence available to Calyx and should be revised when new evidence is added."
                ),
                scientific=False,
            )
        )
        ...
        return article, self.render_markdown(article, bibliography)

    @staticmethod
    def render_markdown(
        article: ArticleDraft, bibliography: tuple[BibliographicRecord, ...]
    ) -> str:
...
class ScientificArticleAuditService:
    """Run grounding validation plus a conservative quantitative support audit."""
    ...
    def audit(
        ...
        *,
        ...
        manifest["state"] = (
            "ARTICLE_AUDIT_PASSED" if manifest["publication_ready"] else "ARTICLE_AUDIT_BLOCKED"
        )
        return manifest
...
class ResearchToArticleMissionService:
    """Governed post-acquisition pipeline from evidence matrix to audited article."""
