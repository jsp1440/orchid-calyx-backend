    ...
    *,
    ...
    with (
        ...
        for item in items:
            ...
            if "technology_scout" in item:
                # Keep the complete versioned assessment with the immutable
                # observation, rather than losing it on the mutable item row.
                snapshot["technology_scout"] = item["technology_scout"]
            cur.execute(
                """
...
def get_intelligence_item(item_id: int) -> dict[str, Any] | None:
    with (
        ...
        cur.execute(
            "SELECT * FROM oc_intake.intelligence_items WHERE id=%s", (item_id,)
        )
        item = cur.fetchone()
        if not item:
            return None
