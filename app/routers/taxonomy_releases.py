...
def create_taxonomy_release_router(
    ...
    router = APIRouter(prefix="/api/mission-control/taxonomy/releases", tags=["taxonomy-releases"])

    @router.get("")
    def list_releases(_: Any = Depends(require_owner)) -> dict[str, Any]:
        return {"releases": get_store().list_reports(), "automatic_promotion": False}

    @router.get("/{release_id}")
    def get_release(release_id: str, _: Any = Depends(require_owner)) -> dict[str, Any]:
