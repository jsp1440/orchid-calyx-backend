...
def corpus():
 ...
 for i,typ,text,rev,policy,extra in rows:
  d={"index_document_id":i,"source_object_type":typ,"source_object_id":i,"revision_id":rev,"parent_type":typ,"parent_id":i,"anchors":(i*10,),"content_hash":sha256(text.encode()).hexdigest(),"metadata_hash":str(i),"model_id":model,"configuration_hash":"c","active":True,"version":1,"metadata":{"display_policy":policy,"document_id":f"document-{i}","document_title":text,"title":text,"document_class":"EDUCATIONAL_MATERIAL" if i==4 else "PRIMARY_RESEARCH","collections":["GENERAL_BRAIN"],"verification_state":"VERIFIED","locator":{"page":i},"anchor_locators":{str(i*10):{"page":i,"paragraph":1}},**extra}}; r.documents.append(d); r.lexical.append({"index_document_id":i,"normalized_text":text.casefold(),"verbatim_text":text,"language":"en","title":text}); r.vectors.append({"index_document_id":i,"vector":p.embed_batch([text])[0],"active":True}); parents[(typ,i)]={"complete_text":text*30,"internal_access_allowed":extra.get("internal_access_allowed",False),"components":["table","figure","caption"]};
 return r,p,parents
...
def test_exact_verbatim_excerpt_hashes_anchor_locator_and_document_scope():
 r,p,parents=corpus(); result=RetrievalEngine(r,p,parents).search(RetrievalQuery("asymbiotic",mode="LEXICAL",filters={"document_id":"document-1"}))["results"][0]
 assert result["authorized_excerpt"]=="Complete asymbiotic germination protocol"
 assert result["source_content_hash"]==sha256(result["authorized_excerpt"].encode()).hexdigest()
 assert result["excerpt_content_hash"]==sha256(result["authorized_excerpt"].encode()).hexdigest()
 assert result["citation"]["document_id"]=="document-1"
 assert result["citation"]["source_anchors"]==[{"anchor_id":10,"locator":{"page":1,"paragraph":1}}]
 preview=RetrievalEngine(r,p,parents).search(RetrievalQuery("teaching",mode="LEXICAL"))["results"][0]
 assert preview["authorized_excerpt"]=="educational "
 assert preview["excerpt_content_hash"]==sha256(b"educational ").hexdigest()
def test_legacy_lexical_row_remains_metadata_retrievable_but_evidence_fails_closed():
 r,p,parents=corpus(); r.lexical[0].pop("verbatim_text")
 result=RetrievalEngine(r,p,parents).search(RetrievalQuery("asymbiotic",mode="LEXICAL",filters={"document_id":"document-1"}))["results"][0]
 assert result["citation"]["document_id"]=="document-1"
 assert result["citation"]["source_anchor_ids"]==[10]
