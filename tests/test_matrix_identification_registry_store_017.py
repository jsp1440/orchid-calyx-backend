...
def test_migration_613_contract_requires_columns_primary_key_and_indexes():
    ready = assess_registry_schema(columns=dict(REQUIRED_COLUMNS), primary_key_columns=["registry_id", "version"], indexes=set(REQUIRED_INDEXES))
    assert ready["migration_613_schema_ready"] is True
    missing = set(REQUIRED_INDEXES)
    missing.remove("idx_matrix_registry_checksum")
    blocked = assess_registry_schema(columns=dict(REQUIRED_COLUMNS), primary_key_columns=["registry_id", "version"], indexes=missing)
...
def test_postgres_registry_store_rejects_partial_schema_before_read(monkeypatch):
    ...
    monkeypatch.setattr(store, "schema_inspection", lambda: {"migration_613_schema_ready": False, "blockers": ["MATRIX_REGISTRY_REQUIRED_COLUMNS_MISSING"]})
...
def test_postgres_registry_store_rejects_partial_schema_before_write(monkeypatch):
    store = PostgresMatrixRegistryStore("postgresql://example.invalid/test")
    monkeypatch.setattr(store, "schema_inspection", lambda: {"migration_613_schema_ready": False, "blockers": ["MATRIX_REGISTRY_PRIMARY_KEY_MISMATCH"]})
