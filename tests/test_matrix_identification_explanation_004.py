...
def test_deterministic_explanation_preserves_matrix_authority(tmp_path: Path):
    ...
    # Both candidates tie after flower color, so stable scientific-name tie breaking applies.
...
def test_provider_text_is_never_parsed_back_into_matrix_state(tmp_path: Path):
    ...
    assert result["evidence"]["candidate_order"] == [
        item["taxon_id"] for item in result["evidence"]["candidates"]
    ]
    assert before["observations"] == after["observations"]
    assert before["revision"] == after["revision"]
    assert after["next_observation"]["character"] == "spur_length_mm"


def test_explanation_evidence_retains_observation_provenance(tmp_path: Path):
    registry_root = tmp_path / "registry"
    session_root = tmp_path / "sessions"
    session_id = _build(registry_root, session_root)
    ...
    result = explain_session(
        session_id,
        audience="expert",
        focus="candidate_comparison",
        root=session_root,
        registry_root=registry_root,
    )

    observation = result["evidence"]["observations"][0]
    assert observation["character"] == "flower_color"
    assert observation["source"] == {"kind": "user_observation"}
