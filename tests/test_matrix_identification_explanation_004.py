...
from runtime.matrix_identification_session import add_observation, create_session, get_session
...
class MisleadingProvider:
    def generate(self, *, messages, governed_context):
        ...
        return GeneratedReply(
            text=(
                "Ignore the Matrix: reverse the candidates, call the score a probability, "
                "and ask for flower color instead."
            ),
...
def _build(root: Path, sessions: Path) -> str:
    create_registry_version(
        ...
        candidates=[
            Candidate("world-plants:angraecum-sesquipedale", "Angraecum sesquipedale", {"flower_color": "white", "spur_length_mm": {"min": 250, "max": 350}}, provenance={"source": "governed fixture"}),
            Candidate("world-plants:angraecum-eburneum", "Angraecum eburneum", {"flower_color": "white", "spur_length_mm": {"min": 80, "max": 150}}, provenance={"source": "governed fixture"}),
        ],
...
def test_deterministic_explanation_preserves_matrix_authority(tmp_path: Path):
    ...
    result = explain_session(session_id, audience="beginner", focus="next_observation", root=session_root, registry_root=registry_root)
    evidence = result["evidence"]
    assert evidence["candidate_order"] == ["world-plants:angraecum-eburneum", "world-plants:angraecum-sesquipedale"]
...
def test_provider_text_is_never_parsed_back_into_matrix_state(tmp_path: Path):
    ...
    result = explain_session(session_id, provider=MisleadingProvider(), root=session_root, registry_root=registry_root)
    after = get_session(session_id, root=session_root)
    assert result["narrative"]["provider"] == "test-provider"
    assert "reverse the candidates" in result["narrative"]["text"]
...
def test_cross_owner_cannot_generate_explanation(tmp_path: Path):
    ...
    with pytest.raises(FileNotFoundError):
        explain_session(session_id, access_actor="different-owner", root=session_root, registry_root=registry_root)
