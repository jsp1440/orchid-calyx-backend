...
def test_duplicate_dispatch_fingerprint_is_rejected():
    report = evaluate({"issues": [], "dispatch_fingerprints": ["301:abc", "301:abc"]})
    assert report["healthy"] is False
    assert report["violations"] == [{"type": "duplicate_dispatch_fingerprint", "fingerprint": "301:abc"}]
...
def test_validating_requires_exact_head():
    ...
    good = evaluate({"issues": [issue(6, "oc-validating", validation_target={"pr": 77, "head_sha": "deadbeef"})]})
