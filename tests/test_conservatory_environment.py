...
class TestMeasuredMeansAnInstrumentSaidSo:
    def test_a_measured_reading_records_its_instrument(self, tmp_path: Path):
        reading = _store(tmp_path).record(
            ...
            value=18.4,
            origin="measured",
            instrument="SensorPush HT.w #A31",
            observed_at=AT,
        )
    ...
    def test_measured_without_an_instrument_is_refused_not_downgraded(
        self, tmp_path: Path
    ):
    ...
    def test_only_a_measured_reading_may_carry_an_instrument(self, tmp_path: Path):
        ...
        for origin in ["manual", "inferred", "unknown"]:
            with pytest.raises(
                EnvironmentError_, match="ONLY_MEASURED_CARRIES_AN_INSTRUMENT"
            ):
                _store(tmp_path).record(
                    location_id="loc-1",
                    variable="temperature_c",
                    ...
                    origin=origin,
                    instrument="Probe 1",
                    derived_from="x" if origin == "inferred" else None,
                    observed_at=AT,
                )
...
class TestInferenceMustBeTraceable:
    def test_an_inferred_value_records_what_it_came_from(self, tmp_path: Path):
        reading = _store(tmp_path).record(
            location_id="loc-1",
            variable="daily_light_integral_mol_m2_d",
            value=8.1,
            origin="inferred",
            derived_from="PPFD readings 2026-08-22, 12h photoperiod",
            observed_at=AT,
        )
...
class TestMissingIsNotZero:
    def test_unknown_cannot_carry_a_value(self, tmp_path: Path):
        with pytest.raises(
            EnvironmentError_, match="UNKNOWN_ORIGIN_CANNOT_CARRY_A_VALUE"
        ):
            _store(tmp_path).record(
                location_id="loc-1",
...
class TestOriginSurvivesIntoTheContext:
    ...
    def test_a_later_manual_reading_does_not_inherit_an_earlier_instrument(
        self, tmp_path: Path
    ):
        store = _store(tmp_path)
        store.record(
...
class TestReadingsSurviveTheStoreFile:
    def test_readings_are_ordered_by_observation_time(self, tmp_path: Path):
        ...
        assert (
            reopened.context_for("loc-1")["variables"]["temperature_c"]["value"] == 21.0
        )
    ...
    def test_readings_are_scoped_to_their_location(self, tmp_path: Path):
        ...
        assert (
            store.context_for("loc-2")["variables"]["temperature_c"]["known"] is False
        )
...
class TestThroughTheApi:
    ...
    def test_a_measurement_without_an_instrument_is_rejected_by_the_api(
        self, tmp_path: Path
    ):
        client, locations = self._client(tmp_path)
    ...
    def test_environment_cannot_be_attached_to_a_location_that_does_not_exist(
        self, tmp_path: Path
    ):
    ...
    def test_environment_routes_require_an_owner(self, tmp_path: Path):
        ...
        assert (
            client.get("/api/conservatory/locations/x/environment").status_code == 401
        )
        assert (
            client.post(
