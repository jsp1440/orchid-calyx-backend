...
def test_bounded_scope_rejects_semantically_empty_mapping_bounds():
    for environments in (
        {"temperature": None},
        {"temperature": []},
        {"temperature": {}},
        ...
        with pytest.raises(
            ...
            CausalScope.model_validate(
                {"scope_class": "bounded", "environments": environments}
            )
...
def test_mapping_scope_normalization_canonicalizes_categorical_text():
    first = normalize_causal_scope(
        {
            "scope_class": "bounded",
    ...
    assert first["scope_id"] == second["scope_id"]
    assert first["environments"] == {"temperature": "cool nights"}
...
def test_mapping_case_difference_cannot_split_real_contradiction():
    repository, service = components()
    scope_a = {
        "scope_class": "bounded",
        "environments": {"Temperature": "Cool Nights"},
    ...
    handoff_mechanistic_candidate(
        request("promotes", scope_a, "mapping-positive"), (repository, service)
    )
    handoff_mechanistic_candidate(
        request("inhibits", scope_b, "mapping-negative"), (repository, service)
    )
    report = analyze_mechanistic_contradictions((repository, service))
    assert report["contradiction_count"] == 1
