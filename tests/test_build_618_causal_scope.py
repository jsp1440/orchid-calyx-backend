...
from app.brain.mechanistic_candidates import MechanisticCandidateRequest, handoff_mechanistic_candidate
...
def request(relationship="promotes", causal_scope=None, reasoning_id="scope:001"):
    return MechanisticCandidateRequest.model_validate({
        "reasoning_id": reasoning_id,
        "source": {"node_type": "environment", "label": "Cool nights", "stable_key": "cool-nights"},
        "relationship": relationship,
        "target": {"node_type": "physiology", "label": "Respiration rate", "stable_key": "respiration-rate"},
        "confidence": 0.8,
...
def approve(repository, candidate_id):
    for review_id, review in list(repository.reviews.items()):
        if review.get("candidate_id") == candidate_id and review.get("state") == "OPEN":
            repository.resolve_review(review_id, "APPROVE_CANDIDATE", "Scope and evidence reviewed.", "qualified.science-reviewer")
...
def test_bounded_scope_requires_real_bounds():
    with pytest.raises(ValueError, match="BOUNDED_CAUSAL_SCOPE_REQUIRES_APPLICABILITY_BOUNDS"):
...
def test_scope_normalization_is_order_independent():
    first = normalize_causal_scope({"scope_class": "bounded", "taxa": ["Phalaenopsis", "Cattleya"], "tissues": ["Leaf", "Root"]})
    second = normalize_causal_scope({"scope_class": "bounded", "taxa": ["cattleya", "phalaenopsis"], "tissues": ["root", "leaf"]})
...
def test_opposite_polarity_in_different_tissues_is_not_a_contradiction():
    ...
    handoff_mechanistic_candidate(request("promotes", leaf, "leaf"), (repository, service))
    handoff_mechanistic_candidate(request("inhibits", root, "root"), (repository, service))
...
def test_opposite_polarity_in_same_normalized_scope_is_a_contradiction():
    repository, service = components()
    scope_a = {"scope_class": "bounded", "taxa": ["Phalaenopsis"], "tissues": ["Leaf"]}
    scope_b = {"scope_class": "bounded", "taxa": ["phalaenopsis"], "tissues": ["leaf"]}
    handoff_mechanistic_candidate(request("promotes", scope_a, "positive"), (repository, service))
    handoff_mechanistic_candidate(request("inhibits", scope_b, "negative"), (repository, service))
    report = analyze_mechanistic_contradictions((repository, service))
    assert report["contradiction_count"] == 1
    assert report["contradictions"][0]["scope"]["causal_scope"]["scope_class"] == "bounded"
