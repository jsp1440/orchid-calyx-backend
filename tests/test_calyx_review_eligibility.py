...
def decision(decision_id: str, review_class: ReviewClass, reviewer_id: str, state: ReviewDecisionState = ReviewDecisionState.APPROVED) -> ReviewDecision:
...
def test_rejection_and_changes_requested_prevent_eligibility():
    registry = ReviewRegistry()
    registry.request(request())
    registry.decide(decision("d1", ReviewClass.SCIENTIFIC, "scientist", ReviewDecisionState.CHANGES_REQUESTED))
...
def test_decisions_are_immutable_and_one_authoritative_decision_per_class():
    ...
    with pytest.raises(ValueError, match="AUTHORITATIVE_REVIEW_ALREADY_RECORDED"):
        registry.decide(decision("d2", ReviewClass.SCIENTIFIC, "other-scientist", ReviewDecisionState.REJECTED))
