...
def test_selected_unregistered_adapter_cannot_become_ready_from_environment():
    result = provider_readiness(
        registry=VisionProviderRegistry(),
        environ={"CALYX_VISION_PROVIDER": "imaginary-provider", "CALYX_VISION_LIVE_INFERENCE_ENABLED": "true"},
    )
...
def test_nonproduction_adapter_is_visible_but_cannot_enable_live_inference():
    ...
    result = provider_readiness(
        registry=registry,
        environ={"CALYX_VISION_PROVIDER": "fixture", "CALYX_VISION_LIVE_INFERENCE_ENABLED": "true"},
    )
...
def test_registered_production_adapter_preserves_specific_readiness_blocker():
    registry = VisionProviderRegistry()
    registry.register(_registration(probe=lambda: ProviderProbeResult(ready=False, code="PROVIDER_CREDENTIALS_NOT_CONFIGURED")))
    result = provider_readiness(registry=registry, environ={"CALYX_VISION_PROVIDER": "dummy"})
...
def test_live_inference_requires_ready_adapter_and_explicit_flag():
    registry = VisionProviderRegistry()
    registry.register(_registration())
    disabled = provider_readiness(registry=registry, environ={"CALYX_VISION_PROVIDER": "dummy"})
    enabled = provider_readiness(registry=registry, environ={"CALYX_VISION_PROVIDER": "dummy", "CALYX_VISION_LIVE_INFERENCE_ENABLED": "true"})
