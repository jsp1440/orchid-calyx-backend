...
def test_missing_index_blocks_schema_readiness():
    ...
    removed = min(REQUIRED_INDEX_COLUMNS)
...
def test_partial_same_name_index_does_not_satisfy_contract():
    snapshot = _valid_snapshot()
...
def test_missing_revision_check_blocks_schema_readiness():
    snapshot = _valid_snapshot()
...
def test_wrong_primary_key_and_column_type_fail_closed():
    snapshot = _valid_snapshot()
