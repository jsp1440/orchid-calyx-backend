...
from runtime.matrix_identification_session import add_observation, create_session
...
def test_new_revision_creates_new_report_without_rewriting_prior_report(tmp_path: Path):
    ...
    loaded_first = get_report(
        session_id,
        first["report_id"],
        access_actor="owner-a",
        root=session_root,
    )
    assert loaded_first["core"]["session_revision"] == 1


def test_report_preserves_candidate_score_and_coverage_as_separate_fields(tmp_path: Path):
    session_id, session_root, registry_root = _setup(tmp_path)
    ...
    candidate = report["core"]["ranking"]["candidates"][0]
    assert "score" in candidate
    assert "coverage" in candidate
    assert report["core"]["ranking"]["disclaimer"].startswith("Scores are candidate-ranking evidence")
