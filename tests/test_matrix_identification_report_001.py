...
def _setup(tmp_path: Path):
    ...
    create_registry_version(
        ...
        characters=[RegistryCharacter("flower_color", "Flower color", weight=1), RegistryCharacter("spur_length_mm", "Spur length", value_type="numeric_range", weight=3)],
        candidates=[Candidate("t1", "Taxon alpha", {"flower_color": "white", "spur_length_mm": {"min": 250, "max": 350}}), Candidate("t2", "Taxon beta", {"flower_color": "white", "spur_length_mm": {"min": 80, "max": 150}})],
        provenance={"source": "test"}, actor="pytest", root=registry_root,
    )
    session = create_session(registry_id="report-demo", version="1", actor="owner-a", root=session_root, registry_root=registry_root)
    add_observation(session["session_id"], character="flower_color", value="white", actor="owner-a", access_actor="owner-a", source={"kind": "user_observation", "interface": "test"}, root=session_root, registry_root=registry_root)
...
def test_report_is_content_addressed_and_idempotent_for_same_revision(tmp_path: Path):
    ...
    assert first["created"] is True; assert second["created"] is False
    ...
    assert core["session_revision"] == 1; assert core["registry"]["checksum_sha256"]
...
def test_new_revision_creates_new_report_without_rewriting_prior_report(tmp_path: Path):
    ...
    add_observation(session_id, character="spur_length_mm", value=300, actor="owner-a", access_actor="owner-a", root=session_root, registry_root=registry_root)
    second = finalize_report(session_id, access_actor="owner-a", root=session_root, registry_root=registry_root)["report"]
    assert second["report_id"] != first["report_id"]; assert second["core"]["session_revision"] == 2
...
def test_report_preserves_score_coverage_and_vision_review_boundary(tmp_path: Path):
    ...
    session["vision_suggestions"] = [{"suggestion_id": "vision-s1", "analysis_id": "analysis-1", "vision_observation_id": "obs-v1", "image_id": "image-1", "character": "spur_length_mm", "state": "pending_review", "proposed_value": 300.0, "accepted_value": None, "machine_confidence": 0.92, "measurement_basis": "CALIBRATED_SCALE", "vision_review_state": "MACHINE_GENERATED", "matrix_observation_id": None, "review": None, "limitations": []}]
    ...
    core = report["core"]; candidate = core["ranking"]["candidates"][0]
    assert "score" in candidate and "coverage" in candidate; assert len(core["observations"]) == 1
...
def test_cross_owner_cannot_finalize_list_or_fetch_report(tmp_path: Path):
    session_id, session_root, registry_root = _setup(tmp_path)
    report = finalize_report(session_id, access_actor="owner-a", root=session_root, registry_root=registry_root)["report"]
    with pytest.raises(FileNotFoundError, match="identification session not found"): finalize_report(session_id, access_actor="owner-b", root=session_root, registry_root=registry_root)
    with pytest.raises(FileNotFoundError, match="identification session not found"): list_reports(session_id, access_actor="owner-b", root=session_root)
    with pytest.raises(FileNotFoundError, match="identification session not found"): get_report(session_id, report["report_id"], access_actor="owner-b", root=session_root)
