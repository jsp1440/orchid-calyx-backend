...
def test_matrix_workspace_output_is_bounded_derived_ranking():
    report = {
        "observation_count": 3,
        "compared_character_count": 3,
        "candidates": [
            {
                "taxon_id": f"taxon-{index}",
...
def test_evaluate_preserves_ranking_and_returns_one_derived_workspace_panel():
    payload = matrix_identification.IdentificationRequest(
        observations=[
            matrix_identification.ObservationInput(
                character="flower_color", value="white", certainty="certain", weight=1
            )
        ],
        candidates=[
            matrix_identification.CandidateInput(
                taxon_id="t1",
                scientific_name="Taxon alpha",
                states={"flower_color": "white"},
                provenance={"source": "test governed matrix"},
            ),
            matrix_identification.CandidateInput(
                taxon_id="t2",
                scientific_name="Taxon beta",
                states={"flower_color": "red"},
                provenance={"source": "test governed matrix"},
            ),
        ],
        limit=20,
