...
class GuardedMigrationRunnerContractTests(unittest.TestCase):
    ...
    def test_blocked_preflight_never_connects(self) -> None:
        database_url = "postgresql://db.example.org/orchid"
        blocked = {**READY_PREFLIGHT, "ready_to_apply_migration": False, "migration_blockers": ["release evidence artifact is invalid"]}
    ...
    def test_apply_uses_bounded_connect_and_transaction_timeouts(self) -> None:
        ...
        invalid_state = {"schema_valid": False, "missing_columns": {"lab_sessions": ["session_id"]}, "missing_constraint_fragments": []}
        valid_state = {"schema_valid": True, "missing_columns": {}, "missing_constraint_fragments": []}
        with patch.object(runner, "preflight", return_value=READY_PREFLIGHT), patch.object(runner.psycopg, "connect", connect), patch.object(runner, "_schema_state_on_connection", side_effect=[invalid_state, valid_state]):
            result = runner.apply_migration(
                release_evidence=Path("evidence.json"),
                database_url=database_url,
                confirm_migration_sha256=runner.migration_digest(),
        ...
        connect.assert_called_once_with(database_url, autocommit=True, connect_timeout=runner.CONNECT_TIMEOUT_SECONDS)
        cursor.execute.assert_any_call("SELECT set_config('lock_timeout', %s, true)", (f"{runner.LOCK_TIMEOUT_MS}ms",))
        cursor.execute.assert_any_call("SELECT set_config('statement_timeout', %s, true)", (f"{runner.STATEMENT_TIMEOUT_MS}ms",))
    ...
    def test_concurrent_winner_becomes_noop_after_advisory_lock(self) -> None:
        ...
        conn.transaction.return_value.__enter__.return_value = MagicMock()
        valid_state = {"schema_valid": True, "missing_columns": {}, "missing_constraint_fragments": []}
        with patch.object(runner, "preflight", return_value=READY_PREFLIGHT), patch.object(runner.psycopg, "connect", connect), patch.object(runner, "_schema_state_on_connection", return_value=valid_state):
            result = runner.apply_migration(
                release_evidence=Path("evidence.json"),
                database_url=database_url,
                confirm_migration_sha256=runner.migration_digest(),
        ...
        self.assertFalse(result["mutations_performed"])
        cursor.execute.assert_any_call("SELECT pg_advisory_xact_lock(%s)", (runner.ADVISORY_LOCK_KEY,))
        migration_sql = runner.MIGRATION.read_text(encoding="utf-8")
        self.assertTrue(all(call_item.args[0] != migration_sql for call_item in cursor.execute.call_args_list))
...
class SchemaValidationNullabilityTests(unittest.TestCase):
    ...
    def test_nullable_publication_control_column_fails_schema_check(self) -> None:
        rows = [("lab_sessions", col, "NO") for col in runner.REQUIRED_COLUMNS["lab_sessions"] if col != "human_review_required"] + [("lab_sessions", "human_review_required", "YES")]
        for tbl, cols in runner.REQUIRED_COLUMNS.items():
            if tbl != "lab_sessions":
                rows += [(tbl, col, "NO") for col in cols]
        constraint_text = " ".join(f"check ({frag})" for frag in runner.REQUIRED_CONSTRAINT_FRAGMENTS)
    ...
    def test_all_required_not_null_columns_pass(self) -> None:
        rows = [(tbl, col, "NO") for tbl, cols in runner.REQUIRED_COLUMNS.items() for col in cols]
        constraint_text = " ".join(f"check ({frag})" for frag in runner.REQUIRED_CONSTRAINT_FRAGMENTS)
        conn = self._make_conn(rows, [(constraint_text,)])
        state = runner._schema_state_on_connection(conn)
