...
def test_every_architecture_has_intent_and_decision_coverage() -> None:
    registry = build_canonical_brain_fixture()
    for architecture in [record for record in registry.snapshot().objects if record.object_type == "architecture"]:
        assert registry.aligned_intents(architecture.object_id)
        decisions = [related for related in registry.related(architecture.object_id, "documents") if related.object_type == "decision"]
        assert decisions
