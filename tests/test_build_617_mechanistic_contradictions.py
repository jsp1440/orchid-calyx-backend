...
def test_analysis_is_read_only_over_candidate_knowledge_state():
    repository, service = components()
    handoff_mechanistic_candidate(payload("promotes"), (repository, service))
    handoff_mechanistic_candidate(payload("inhibits"), (repository, service))
    before = {
        "candidates": deepcopy(repository.candidates),
        "reviews": deepcopy(repository.reviews),
        "conflicts": deepcopy(repository.conflicts),
        "evidence_links": deepcopy(repository.evidence_links),
    }

    report = analyze_mechanistic_contradictions((repository, service))

    assert report["candidate_mutation"] is False
    assert report["graph_mutation"] is False
    assert repository.candidates == before["candidates"]
    assert repository.reviews == before["reviews"]
    assert repository.conflicts == before["conflicts"]
    assert repository.evidence_links == before["evidence_links"]
