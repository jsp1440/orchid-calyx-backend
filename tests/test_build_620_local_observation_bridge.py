...
def test_local_observation_is_noncausal_review_required_candidate():
    ...
    assert result["graph_preview"]["semantics"] == {"role": "evidence", "polarity": 0, "causal": False}
...
def test_repeated_same_plant_observations_remain_distinct_longitudinal_events():
    ...
    second = handoff_local_observation(request("obs-002", response_label="Second root tip emerged"), (repository, service))
...
def test_local_history_returns_only_same_plant_local_observations():
    ...
    handoff_local_observation(request("obs-007", response_label="Leaf expanded"), (repository, service))
