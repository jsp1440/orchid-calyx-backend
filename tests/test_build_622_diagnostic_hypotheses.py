...
def test_applicable_paths_become_ranked_hypotheses():
    ...
    assert all(
        item["applicability_status"] == "applicable" for item in result["hypotheses"]
    )
...
def test_indeterminate_scope_is_penalized_and_remains_unresolved():
    ...
    assert result["hypothesis_count"] > 0
    assert all(
        item["applicability_status"] == "indeterminate" for item in result["hypotheses"]
    )
    assert all(
        item["scoring_basis"] == "scope_indeterminate_penalty"
