...
def test_current_parent_executor_completes_exact_evidence_bound_flow() -> None:
    ...
    assert snapshot["schema"] == "calyx-git-proposal-mutation-receipt-v2"
...
def test_wrong_push_commit_fails_closed_with_verified_partial_receipt() -> None:
...
def test_remote_failure_preserves_only_verified_completed_actions() -> None:
    store, gate, request, grant, manifest, plan, _ = _execution_inputs()
