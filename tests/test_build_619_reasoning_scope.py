...
from app.brain.scoped_reasoning_map import ScopedReasoningMapRequest, build_scoped_reasoning_map
...
def test_matching_bounded_scope_is_applicable():
    result = evaluate_scope(
        {"scope_class": "bounded", "taxa": ["Phalaenopsis"], "tissues": ["leaf"]},
        {"scope_class": "bounded", "taxa": ["phalaenopsis"], "tissues": ["leaf"]},
    )
...
def test_scoped_reasoning_map_retains_out_of_scope_paths_transparently():
    ...
    assert any(path["applicability"]["status"] == "out_of_scope" for path in result["paths"])
...
def test_global_claim_scope_applies_to_bounded_query():
    result = evaluate_scope(
        ...
        {"scope_class": "bounded", "taxa": ["cattleya"], "tissues": ["leaf"]},
    )
    assert result["status"] == "applicable"
