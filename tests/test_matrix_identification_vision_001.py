...
def test_accepting_suggestion_creates_reviewed_matrix_observation(tmp_path: Path):
    session, session_root, registry_root = _session(tmp_path)
    service = _vision()
...
def test_rejection_preserves_provenance_without_scoring_observation(tmp_path: Path):
    ...
    assert reviewed["observation_added"] is False
    assert reviewed["suggestion"]["state"] == "rejected"
    assert reviewed["session"]["observations"] == []
    assert reviewed["suggestion"]["review"]["comments"] == "structure was obscured"
