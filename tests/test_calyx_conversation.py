...
def test_correlation_analysis():
    result = run_analysis(
        AnalysisRequest(operation="correlation", x=[1, 2, 3], y=[2, 4, 6])
    )
...
def test_covariance_analysis():
    result = run_analysis(
        AnalysisRequest(operation="covariance", x=[1, 2, 3], y=[2, 4, 6])
    )
...
def test_linear_regression_analysis():
    result = run_analysis(
        AnalysisRequest(operation="linear_regression", x=[1, 2, 3], y=[3, 5, 7])
    )
...
def test_percent_change_analysis():
    result = run_analysis(
        AnalysisRequest(operation="percent_change", values=[100, 125])
    )
...
def test_moving_average():
    result = run_analysis(
        AnalysisRequest(operation="moving_average", values=[1, 2, 3, 4, 5], window=3)
    )
...
def test_memory_conversation_store_persists_transcript_and_context():
    ...
    cid = store.create_or_touch(
        None, title="Orchid question", context={"taxon": "Thelymitra"}
    )
