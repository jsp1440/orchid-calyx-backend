...
from app.species_dossier.service import (
    SpeciesDossierService,
    extract_species_name_from_url,
...
def test_extracts_species_from_url_slug():
    assert (
        extract_species_name_from_url("https://example.org/species/Dracula_vampira")
        == "Dracula vampira"
    )
    assert (
        extract_species_name_from_url("https://example.org/species/Dracula_Vampira")
...
def test_resolves_canonical_taxon_id():
    result = SpeciesDossierService(FakeRepository()).resolve(
        FederationResolveRequest(taxon_id="oc:1")
    )
...
def test_resolves_synonym_to_canonical_dossier():
    result = SpeciesDossierService(FakeRepository()).resolve(
        FederationResolveRequest(name="Masdevallia vampira")
    )
    assert result.status == "resolved"
...
def test_partner_slug_uses_one_adaptive_integration():
    result = SpeciesDossierService(FakeRepository()).resolve(
        FederationResolveRequest(
            partner_slug="iospe", partner_species_slug="dracvampira"
        )
    )
    assert result.status == "resolved"
...
def test_ambiguous_name_fails_closed():
    result = SpeciesDossierService(FakeRepository()).resolve(
        FederationResolveRequest(name="Epidendrum ambiguum")
    )
...
def test_unresolved_name_does_not_guess():
    result = SpeciesDossierService(FakeRepository()).resolve(
        FederationResolveRequest(name="Unknown orchid")
    )
