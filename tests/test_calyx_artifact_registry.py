...
def test_lineage_requires_registered_artifacts_and_is_idempotent():
    ...
    first = registry.relate(source_artifact_id="output", relation=ArtifactRelationType.DERIVED_FROM, target_artifact_id="receipt")
    replay = registry.relate(source_artifact_id="output", relation="derived_from", target_artifact_id="receipt")
    assert first == replay
    assert len(registry.lineage("output")) == 1
    with pytest.raises(LookupError, match="ARTIFACT_NOT_FOUND"):
        registry.relate(source_artifact_id="missing", relation="derived_from", target_artifact_id="receipt")
...
def test_missing_evidence_is_rejected_until_evidence_relationship_exists():
    ...
    registry.relate(source_artifact_id="claim", relation=ArtifactRelationType.EVIDENCES, target_artifact_id="evidence")
