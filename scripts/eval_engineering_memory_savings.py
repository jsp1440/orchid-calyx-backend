...
from app.engineering_memory.evaluation import format_report, run_evaluation  # noqa: E402


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--telemetry", help="path to measured telemetry JSON", default=None)
    ...
    if enabled is None or baseline is None or enabled <= baseline:
        print("FAIL: enabled retrieval did not improve relevance over baseline", file=sys.stderr)
        return 1
    return 0
