...
def apply_migration(
    *,
    ...
    _require(snapshot_digest == expected_digest, "migration file changed during guarded apply; regenerate the dry-run plan and confirmations")
    _require(confirm_migration_sha256.strip().lower() == expected_digest, "exact migration SHA-256 confirmation is required")
    _require(confirm_database_target.strip() == expected_target, "exact database target confirmation is required")
    _require(bool(migration_plan["migration_stage_preflight"]["ready"]), "migration-stage preflight is blocked")
    if migration_plan["schema_already_valid"]:
        return {**migration_plan, "mode": "apply", "would_apply": False, "result": "already_valid_noop", "mutations_performed": False}
    try:
    ...
    return {
        **migration_plan,
        "mode": "apply",
        "would_apply": False,
        "result": "applied_and_verified" if mutations_performed else "already_valid_noop_after_lock",
