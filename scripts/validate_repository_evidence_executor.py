...
def run_command(name: str, command: list[str]) -> dict[str, object]:
    ...
    try:
        ...
        return {"name": name, "command": command, "started_at": started_at, "finished_at": utcnow(), "return_code": 127, "stdout_tail": "", "stderr_tail": f"{type(exc).__name__}: {exc}"}
    return {"name": name, "command": command, "started_at": started_at, "finished_at": utcnow(), "return_code": result.returncode, "stdout_tail": result.stdout[-8000:], "stderr_tail": result.stderr[-8000:]}
...
def main() -> int:
    ...
    receipt: dict[str, object] = {
        ...
        "authority": {"merge": False, "deploy": False, "publish": False, "production_write": False},
