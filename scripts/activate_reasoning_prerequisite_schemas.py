...
RESEARCH_STATION_MIGRATIONS = (
    ("101_research_workspace_foundation.sql", "3333853c97832154cb0f61bace0c2184396da160"),
    ("140_calyx_conversation_sessions.sql", "f572ba9aa1a3bade4dfe9d1e1faf0dbfb8a57baf"),
)
...
REQUIRED_COLUMNS: dict[str, frozenset[str]] = {
    ...
    "oc_knowledge_publication.graph_versions": frozenset(
        {"graph_version_id", "sequence", "graph_transaction_id", "status", "fingerprint"}
    ),
...
RS_140_TABLES = ("conversation_sessions", "conversation_messages")
RS_COLUMN_TYPES: dict[str, dict[str, str]] = {
    "projects": {
        "project_id": "uuid",
        "owner_subject": "text",
        "title": "text",
...
RS_NOT_NULL: dict[str, frozenset[str]] = {
...
RS_DEFAULT_FRAGMENTS: dict[str, dict[str, str]] = {
    ...
    "conversation_sessions": {"conversation_id": "gen_random_uuid", "title": "'New conversation'::text", "created_at": "now()", "updated_at": "now()", "version": "1"},
...
RS_140_INDEXES = frozenset(
    {"idx_rs_conversation_owner_project_updated", "idx_rs_conversation_messages_session_time"}
)
RS_CONSTRAINT_FRAGMENTS: dict[str, tuple[str, ...]] = {
    "projects": ("PRIMARY KEY (project_id)",),
    "saved_searches": ("PRIMARY KEY (saved_search_id)", "FOREIGN KEY (project_id) REFERENCES research_station.projects(project_id)"),
    "notes": ("PRIMARY KEY (note_id)", "FOREIGN KEY (project_id) REFERENCES research_station.projects(project_id)"),
    "project_taxa": ("PRIMARY KEY (project_id, taxon_id)", "FOREIGN KEY (project_id) REFERENCES research_station.projects(project_id)"),
    "project_documents": ("PRIMARY KEY (project_id, document_id)", "FOREIGN KEY (project_id) REFERENCES research_station.projects(project_id)"),
    "project_evidence": ("PRIMARY KEY (project_id, evidence_kind, evidence_id)", "FOREIGN KEY (project_id) REFERENCES research_station.projects(project_id)"),
    "audit_events": ("PRIMARY KEY (event_id)", "FOREIGN KEY (project_id) REFERENCES research_station.projects(project_id)"),
    "conversation_sessions": ("PRIMARY KEY (conversation_id)", "FOREIGN KEY (project_id) REFERENCES research_station.projects(project_id)"),
    "conversation_messages": (
...
def _identity_report(migrations: tuple[tuple[str, str], ...]) -> dict[str, dict[str, Any]]:
...
def _relation_report(connection, qualified_name: str) -> dict[str, Any]:
    ...
    if not exists:
        return {"exists": False, "complete": False, "missing_columns": sorted(REQUIRED_COLUMNS[qualified_name])}
...
def _function_names(connection) -> set[str]:
    rows = connection.execute(
        "SELECT n.nspname || '.' || p.proname FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname IN ('oc_knowledge_publication','research_station')"
    ).fetchall()
...
def _trigger_names(connection) -> set[str]:
    rows = connection.execute(
        ...
        WHERE NOT t.tgisinternal AND n.nspname IN ('oc_knowledge_publication','research_station')
        """
    ).fetchall()
    return {row[0] for row in rows}
...
def inspect_contract(connection) -> dict[str, Any]:
    relation_reports = {target: _relation_report(connection, target) for target in REQUIRED_COLUMNS}
    ...
    for migration, targets in MIGRATION_TARGETS.items():
        ...
        stage_reports[migration] = {"exists_any": exists_any, "complete": complete, "malformed": malformed, "targets": list(targets)}
...
def classify_preflight(contract: dict[str, Any], identities_match: bool) -> dict[str, Any]:
    if not identities_match:
        return {"status": "blocked", "activation_required": not contract["complete"], "ready_to_apply": False, "blockers": ["MIGRATION_IDENTITY_DRIFT", *contract["blockers"]]}
    if contract["complete"]:
        return {"status": "passed", "activation_required": False, "ready_to_apply": False, "blockers": []}
    if not contract["safe_resume"] or contract["blockers"]:
        return {"status": "blocked", "activation_required": True, "ready_to_apply": False, "blockers": list(contract["blockers"])}
...
def _rs_table_report(connection, table: str) -> dict[str, Any]:
    expected = RS_COLUMN_TYPES[table]
    ...
    wrong_types = sorted(name for name, data_type in expected.items() if name in actual and actual[name]["data_type"] != data_type)
    nullability_mismatches = sorted(name for name in RS_NOT_NULL[table] if name in actual and actual[name]["is_nullable"] != "NO")
    default_mismatches = sorted(
        name
        for name, fragment in RS_DEFAULT_FRAGMENTS.get(table, {}).items()
        if name in actual and fragment not in str(actual[name]["column_default"])
    )
    complete = not missing and not wrong_types and not nullability_mismatches and not default_mismatches
    return {
        "exists": True,
        "complete": complete,
...
def _rs_index_names(connection) -> set[str]:
    return {row[0] for row in connection.execute("SELECT indexname FROM pg_indexes WHERE schemaname='research_station'").fetchall()}
...
def inspect_research_station_conversation_contract(connection) -> dict[str, Any]:
    tables = {table: _rs_table_report(connection, table) for table in (*RS_101_TABLES, *RS_140_TABLES)}
    prereq_reports = [tables[table] for table in RS_101_TABLES]
    target_reports = [tables[table] for table in RS_140_TABLES]
    ...
    prereq_columns_complete = all(report["complete"] for report in prereq_reports)
    target_columns_complete = all(report["complete"] for report in target_reports)
    blockers: list[str] = []

    if prereq_exists_any and not prereq_columns_complete:
        blockers.append("MALFORMED_OR_PARTIAL_MIGRATION_101_STATE")
    if target_exists_any and not target_columns_complete:
        blockers.append("MALFORMED_OR_PARTIAL_MIGRATION_140_STATE")
    if target_exists_any and not prereq_columns_complete:
        blockers.append("MIGRATION_140_PRESENT_WITHOUT_CANONICAL_101")

    if prereq_columns_complete:
        for table in RS_101_TABLES:
            joined = "\n".join(_rs_constraints(connection, table))
            for fragment in RS_CONSTRAINT_FRAGMENTS[table]:
                if fragment.lower() not in joined.lower():
                    blockers.append(f"MIGRATION_101_CONSTRAINT_MISSING:{table}:{fragment}")
    if target_columns_complete:
        for table in RS_140_TABLES:
            joined = "\n".join(_rs_constraints(connection, table))
            for fragment in RS_CONSTRAINT_FRAGMENTS[table]:
                if fragment.lower() not in joined.lower():
                    blockers.append(f"MIGRATION_140_CONSTRAINT_MISSING:{table}:{fragment}")

    indexes = _rs_index_names(connection)
    missing_101_indexes = sorted(RS_101_INDEXES - indexes) if prereq_columns_complete else []
    missing_140_indexes = sorted(RS_140_INDEXES - indexes) if target_columns_complete else []
    ...
    conversation_triggers = _rs_trigger_locations(connection, "rs_conversation_messages_immutable")
    audit_collision = any((item["schema"], item["table"]) != ("research_station", "audit_events") for item in audit_triggers)
    conversation_collision = any((item["schema"], item["table"]) != ("research_station", "conversation_messages") for item in conversation_triggers)
    if audit_collision:
    ...
    if prereq_columns_complete:
        ...
        if not connection.execute("SELECT to_regprocedure('research_station.reject_audit_mutation()')").fetchone()[0]:
            blockers.append("AUDIT_REJECT_FUNCTION_MISSING")
    if target_columns_complete:
        valid_conversation = [
            item
            for item in conversation_triggers
            if (item["schema"], item["table"]) == ("research_station", "conversation_messages")
            and "BEFORE DELETE OR UPDATE" in item["definition"]
            and "research_station.reject_conversation_message_mutation()" in item["definition"]
        ]
        if len(valid_conversation) != 1 or len(conversation_triggers) != 1:
            blockers.append("CONVERSATION_IMMUTABLE_TRIGGER_INVALID")
        if not connection.execute("SELECT to_regprocedure('research_station.reject_conversation_message_mutation()')").fetchone()[0]:
    ...
    prereq_complete = prereq_columns_complete and not any(
        blocker.startswith("MIGRATION_101_") or blocker.startswith("AUDIT_")
        for blocker in blockers
    )
    target_complete = target_columns_complete and not any(
        blocker.startswith("MIGRATION_140_") or blocker.startswith("CONVERSATION_")
        for blocker in blockers
    )
    ...
    elif not prereq_exists_any and not target_exists_any and not blockers:
        state = "ABSENT"
    else:
...
def classify_research_station_preflight(contract: dict[str, Any], identities_match: bool) -> dict[str, Any]:
    blockers = list(contract["blockers"])
    if not identities_match:
        blockers.insert(0, "MIGRATION_IDENTITY_DRIFT")
    if blockers or not contract["safe_resume"]:
        return {"status": "blocked", "activation_required": not contract["complete"], "ready_to_apply": False, "blockers": sorted(set(blockers))}
    if contract["complete"]:
        return {"status": "passed", "activation_required": False, "ready_to_apply": False, "blockers": []}
    return {"status": "ready", "activation_required": True, "ready_to_apply": True, "blockers": []}


def apply_research_station_conversation_chain(connection, *, inject_failure_after: str | None = None) -> dict[str, Any]:
    ...
    with connection.transaction():
        ...
        if preflight["status"] == "blocked":
            raise RuntimeError("RESEARCH_STATION_PREFLIGHT_BLOCKED:" + ",".join(preflight["blockers"]))
        if preflight["status"] == "passed":
            return {"applied_migrations": [], "contract_before": before, "contract_after": before, "already_complete": True}
        ...
        if not before["migration_101_complete"]:
            ...
            if not after_101["migration_101_complete"] or after_101["blockers"]:
                raise RuntimeError("MIGRATION_101_POSTCONDITION_FAILED:" + ",".join(after_101["blockers"]))
        ...
        if not after["complete"]:
            raise RuntimeError("MIGRATION_140_POSTCONDITION_FAILED:" + ",".join(after["blockers"]))
...
def _run_research_station_profile(database_url: str, apply: bool) -> int:
    ...
    confirmation_present = os.environ.get("CALYX_RESEARCH_STATION_CONFIRM", "").strip() == RESEARCH_STATION_CONFIRMATION
    receipt: dict[str, Any] = {
        ...
        "serialization": {"mechanism": "pg_advisory_xact_lock", "lock_id": POSTGRES_VALIDATION_LOCK_ID},
    ...
    with psycopg.connect(database_url, autocommit=True) as connection:
        ...
        try:
            ...
            receipt["contract_after_rollback"] = inspect_research_station_conversation_contract(connection)
            _write_receipt(receipt)
            return 2
        ...
        receipt["ready_to_apply"] = False
        receipt["blockers"] = [] if receipt["activation_complete"] else ["POST_APPLY_CONTRACT_INCOMPLETE"]
        _write_receipt(receipt)
...
def main() -> int:
    ...
    identities_match = all(item["matches"] for item in identities.values())
    confirmation_present = os.environ.get("CALYX_REASONING_PREREQ_CONFIRM", "").strip() == CONFIRMATION
    receipt: dict[str, Any] = {
        "schema_version": "1.0",
        "captured_at": datetime.now(timezone.utc).isoformat(),
    ...
    with psycopg.connect(database_url, autocommit=True) as connection:
        ...
        for filename, _ in MIGRATIONS:
            try:
                ...
                receipt["migration_results"].append({"migration": filename, "status": "failed", "error_type": type(exc).__name__})
                break
            receipt["applied_migrations"].append(filename)
            receipt["migration_results"].append({"migration": filename, "status": "completed"})
        after = inspect_contract(connection)
        receipt["contract_after"] = after
        receipt["production_database_mutation_observed"] = before != after
        receipt["partial_application"] = bool(receipt["applied_migrations"] and not after["complete"])
        if receipt["failed_migration"] or not after["complete"]:
            receipt["status"] = "blocked"
            receipt["activation_complete"] = False
            if receipt["failed_migration"]:
                receipt["blockers"] = [*receipt["blockers"], f"MIGRATION_FAILED:{receipt['failed_migration']}"]
            if not after["complete"]:
                receipt["blockers"] = [*receipt["blockers"], "POST_APPLY_CONTRACT_INCOMPLETE"]
            _write_receipt(receipt)
            return 2
