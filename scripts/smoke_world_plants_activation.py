...
BASE_URL = os.getenv("CALYX_BACKEND_URL", "https://orchid-calyx-backend.onrender.com").rstrip("/")
...
def _json_request(path: str, *, method: str = "GET", payload: dict | None = None, token: str = "") -> tuple[int, dict]:
...
def _multipart_upload(token: str) -> tuple[int, dict]:
    ...
    fields = {
        ...
        "notes": "Harmless CALYX-TAXONOMY-ACTIVATION-002 smoke fixture; never canonical.",
    }
    chunks: list[bytes] = []
...
def main() -> int:
    ...
    if not FIXTURE.is_file():
        ...
        return 1
    try:
        status, session = _json_request("/api/mission-control/owner/session", method="POST", payload={"access_code": ACCESS_CODE})
        ...
        status, detail = _json_request(f"/api/mission-control/taxonomy/releases/{release_id}", token=token)
        if status != 200 or str(detail.get("release_id") or detail.get("snapshot_id") or "") != release_id:
            raise RuntimeError("SMOKE_READBACK_FAILED")
        status, listing = _json_request("/api/mission-control/taxonomy/releases", token=token)
        release_ids = {str(item.get("release_id") or item.get("snapshot_id") or "") for item in listing.get("releases", [])}
        if status != 200 or release_id not in release_ids:
            raise RuntimeError("SMOKE_RELEASE_NOT_LISTED")
        status, readiness = _json_request("/api/mission-control/taxonomy/readiness", token=token)
        if status != 200:
        ...
        if readiness.get("ready_for_upload") is not True:
            blocked = [gate.get("name") for gate in readiness.get("gates", []) if gate.get("status") != "passed" and gate.get("name") != "owner_promotion_approval"]
